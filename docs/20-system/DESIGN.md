...
<!-- explicit prohibition (LLM 정확도 단일 최대 기여 — ADR-073 D5):
     ...
     - 아이콘 버튼: accessible name 확보 — 브라우저 computed name(aria-label·aria-labelledby·감싼 visible text·alt·title 등 *어느 출처든*; aria-label 강제 아님). 정밀 판정은 실화면 axe(stack-guard source-verified current-v2 `validate:design` adapter·visual QA, ADR-072 D6)
