...
<!-- 각 AC에 대응하는 테스트 파일·테스트 이름. 사람이 미리 채우거나 builder가 Red phase 시작 전에 채운다.
     ...
     **차단하지 않는다** — report와 stabilize telemetry에 분포만 집계한다. 경계(외부 API·DB)를 넘는 AC가
     ...
     - 한 AC = modality 하나(ADR-065 D1). 두 종류 검증이 필요하면 AC를 쪼갠다. -->
...
- Architecture-Iface: <!-- 해당 스택 한정. 예: [ARCH ## 7-1 API](../../20-system/ARCHITECTURE_OVERVIEW.md#arch-7-1) / [## 7-4 프론트](../../20-system/ARCHITECTURE_OVERVIEW.md#arch-7-4) / [## 7-5 모바일](../../20-system/ARCHITECTURE_OVERVIEW.md#arch-7-5). 비해당 스택은 줄 자체 삭제 (placeholder 잔존 X). 정책: ADR-073 D1. -->
