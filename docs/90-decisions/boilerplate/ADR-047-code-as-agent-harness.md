...
- 본 보일러의 적용: **implement-workitem foreman의 file-disjoint slice partition** — foreman이 한 task를 `## 3. 구현 항목`의 step 파일 경로로 나눠 *충돌 없는 slice는 병렬 builder, 겹치거나 작으면 단일/순차*로 운전한다(ADR-051 #d6 re-anchor) (현재 SSOT: ADR-075 D6). **plan-time wave 계산·echo + TASK_TEMPLATE `## 9` 5필드 구조화(`write_set` 등) + ADR-038#amend-3 write_set 메커니즘은 ADR-051 #d5가 *폐지*** (wave 전용 스키마 — 사용자 결정 완전 제거) (현재 SSOT: ADR-075 D5). 적용 surface SSOT: ADR-051 #d6(foreman `## 3`-path partition) (현재 SSOT: ADR-075 D6). (ADR-026 `## 9` 5필드 Surfaces 줄도 함께 제거됨.)
...
- .claude/skills/implement-workitem/SKILL.md                                         — D9 적용 (foreman `## 3` step-path partition — ADR-051 #d6 re-anchor) (현재 SSOT: ADR-075 D6)
