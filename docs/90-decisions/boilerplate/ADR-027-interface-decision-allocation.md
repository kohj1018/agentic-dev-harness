...
### 배경
- [관측됨] `## 7-4. 프론트 결정`의 소항목이 웹 전용이다(SSR-CSR·SEO·쿠키/스토리지 토큰 저장·폼 validation 라이브러리). 모바일 앱에 존재하지 않는 개념이다. (ADR-059 D7이 이 자리를 사용한다)
