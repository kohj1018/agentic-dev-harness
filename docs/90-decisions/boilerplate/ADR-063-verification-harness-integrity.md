...
- probe 생성 자체가 차단되면(권한·sandbox·이름 충돌·**등록된 소스 루트 부재** — 스캐폴드 전이면 둘 자리가 없다) `SKIPPED — probe 생성 불가: <사유>`로 보고하고 **`WIRING FAIL`로 판정하지 않으며 종료하지 않는다**. 소스 루트를 새로 만들지 않는다 — 소스 트리 구조는 스캐폴드·계획의 소관이다. (참조 갱신 2026-09: green-field 스캐폴드는 [ADR-071](ADR-071-stack-decision-catalog-and-scaffold.md) D5가 `/stack-guard` 수행 0으로 소유한다 — probe는 그 뒤에 돌며 여전히 루트를 만들지 않는다.)
