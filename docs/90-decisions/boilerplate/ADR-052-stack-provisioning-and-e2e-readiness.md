...
- [관측됨] graduation checklist(ADR-014 #1 item 3 `E2E Pass (스택에 정의된 경우)`)의 *"정의된 경우"* 단서가 E2E 미정의 스택을 *통과*시켜, E2E가 필요한 스택에서도 readiness 없이 졸업이 가능하다. (현재 SSOT: ADR-067)
- 기존 규약: ADR-025(외부 의존 권장 + CI 권장), ADR-040#amend-1(install authoring=plan / 실행=implement), ADR-014(graduation 5+1). 본 ADR은 stack-guard에 baseline install provision + 정합 검증 boundary를 추가하고 E2E readiness를 hard-block으로 승격한다. (현재 SSOT: ADR-067)
...
### D3. E2E MUST-run hard-block (ADR-014 graduation item 3 amend) (현재 SSOT: ADR-067)
ADR-014 graduation checklist item 3 `E2E Pass (스택에 정의된 경우)`를 **`E2E Pass (E2E-applicable 스택은 MUST, exit code 0)`**로 강화한다. (현재 SSOT: ADR-067)
...
- 본 D3은 ADR-014 `## Amendment 2`로 박는다(아래 Surfaces). (현재 SSOT: ADR-067)
...
1. **Target** — `.claude/skills/stack-guard/SKILL.md`(install provision + e2e provision/smoke); `.claude/skills/stabilize-milestone/SKILL.md`(graduation pre-check item 3 hard-block); `.claude/skills/repair-milestone/SKILL.md` 신규; ADR-014 `## Amendment 2` + `## Surfaces`; ADR-025·ADR-040 boundary 참조; STRUCTURE.md roster. (현재 SSOT: ADR-067)
...
6. **Rollback path** — 본 ADR superseded → stack-guard install provision·e2e provision/smoke 단계 제거(기존 "기본 설치 안 함" 복원), ADR-014 item 3 "정의된 경우" 복원(`## Amendment 2` 철회), repair-milestone skill 제거(양 mirror). (현재 SSOT: ADR-067)

## 정책 강도 (ADR-022)
- D1·D2: enabling→install-by-default + 실패 시 blocker(약~중 — 설치를 *수행*하되 실패를 fabricate 않고 Needs Install로 막음, 자동 차단은 설치 실패 시에만). D4: enabling(약) — repair 라우팅 + 코드 수정(커밋·status X). D3: constraint(강) — E2E-applicable 스택에 graduation hard-block. **Evidence label**: `[관측됨]`(미설치발 검증 에러·readiness 미검출 관측) + ADR-014 [외부실증] 계승. (현재 SSOT: ADR-067)
...
- docs/90-decisions/boilerplate/ADR-014-milestone-graduation.md   — `## Amendment 2` + Surfaces add (현재 SSOT: ADR-067)
...
## 참고
- ADR-014(graduation — item 3 hard-block amend), ADR-025(외부 의존 + CI 권장 — install/e2e provision 확장), ADR-040(install authoring/실행 — stack-guard 검증 추가로 3분할), ADR-047(harness mutation + D1 Executability), ADR-050(de-fork — repair-milestone 메인 세션 정합), ADR-051(foreman/fan-out — stabilize fan-out과 정합). (현재 SSOT: ADR-067)
...
- docs/90-decisions/boilerplate/ADR-014-milestone-graduation.md — Amendment 4 (현재 SSOT: ADR-067)
