...
| 004 | Model alias policy | accepted | (+#amend-1: agent 이름 역할 중심, +#amend-2: 도구 무관 비고정 + 추론 강도 포함, +#amend-3: Claude shared 파일에서도 모델 키 제거, +#amend-4: agent frontmatter effort 허용 + builder 실험) | shared 도구 설정 파일에 모델·추론 강도 키를 두지 않는다 (별칭은 agent frontmatter에서만) |
