...
| 014 | Milestone graduation contract | accepted | (+#amend-1: evaluator-optimizer pattern 명명, +#amend-2: E2E MUST-run hard-block — ADR-052 D3, +#amend-3: 회고 graduation 판정 줄 — 로드맵 파생 입력, +#amend-4: 0-spec 예외 철회 — 실행된 e2e 1개 이상 성공) | graduation checklist 5+1 + 회고 + pre-check + --dry-run |
...
| 052 | Stack provisioning (install) + E2E readiness | accepted | (+#amend-1: 0-spec 판정 정합화 — 5상태 분류, 문자열 매칭 폐기) | stack-guard가 baseline toolchain·e2e 직접 install/provision(실패 시 Needs Install blocker) + 정합 검증 + e2e provision/smoke + E2E MUST-run hard-block(ADR-014#amend-2) + repair-milestone 신설(코드수정 허용·커밋 X) |
