...
본 ADR이 `.claude/skills` / `.claude/agents` / `AGENTS.md` / `.agents/skills` / `.codex/config.toml` / lifecycle ADR 중 *어느 하나라도* 수정한다면 ([ADR-047](ADR-047-code-as-agent-harness.md) D3 대상 surface), 본문에 `## Mutation Contract` 섹션 **7 필드**(Target / Failure mode / Predicted improvement / Preserved invariants / Falsifying evaluation / Rollback path / **예산 영향**)를 명시한다. **7번째 「예산 영향」**(ADR-047#amend-3): 이 변경이 어떤 위임 단위(sub-agent)의 작업량을 늘리는가, 그 단위의 `maxTurns`·slice 기준을 함께 보았는가. 늘리지 않으면 `없음` 한 줄 — **`없음` 이 정상이고 대다수다**. 이 필드의 값은 쓰는 사람이 한 번 멈춰 보게 하는 데 있다(관측 5건: 규칙을 늘린 뒤 그 규칙을 수행할 에이전트가 산출물 직전에 조용히 죽었다). ADR-022와 양립 — evidence label은 그대로, Mutation Contract는 변경 governance 양식.
