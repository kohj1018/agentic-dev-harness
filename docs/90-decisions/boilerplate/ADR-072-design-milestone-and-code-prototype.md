...
- **`fonts[]` (선택 — schema v1 minor, 2026-09-12)**: 매니페스트 최상위에 **DESIGN `## 3` 이 확정한 글꼴 패밀리 이름**을 적는다(`["Pretendard Variable"]`). 게이트가 두 검사를 건다 — **1차(차단)**: 그 패밀리의 **선언**이 실재하는가(`@font-face`·`next/font/local`·`pubspec.yaml fonts:`). **2차(보고)**: 렌더 직후 `document.fonts.check('16px "<family>"')`. 없으면 두 검사를 건너뛴다(기존 매니페스트 호환). **왜 필요한가**: 글꼴이 없는 채로 찍힌 기준선은 「승인본 = 제품」 등식을 깨는데 **그림 자체는 정상으로 보여** 사람이 못 알아챈다 — Round 12 실측: 웹 승인 스냅샷 26장이 fallback 글꼴로 찍혔고, 같은 공백이 Flutter 에서는 tofu 로 났다(발견 55·69, ADR-073#amend-1).
