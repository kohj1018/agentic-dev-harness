...
- [ ] `/bootstrap-stack`을 실행했다 — 스택이 정해졌으면 `[스택 설명]`을 넘겨 문서화, 미정이면 무입력으로 심층 결정 라운드
  ...
  ```
- [ ] `STACK_SETUP_PLAN.md`를 검토한 뒤 `/stack-guard`를 실행해 통합 `validate` 진입점·verify 스크립트를 생성했다. UI 판정이면 `## Design Gate Adapter`가 `ready`이고 10-case self-test를 통과했는지 확인했다(ADR-058#amend-1)
