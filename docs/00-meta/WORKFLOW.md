...
- UI 프로젝트의 `/bootstrap-design` 라운드 구조는 ADR-058(design workflow): R0(evidence-on-demand 리서치 + `DESIGN_RESEARCH.md`) → R1(원칙 + voice 기본값 확인 — ADR-073 D6) → **R2(DESIGN.md 작성 *전* 다중 concept 시안 REFINE/EXPLORE — 실카피 렌더 + 수용 게이트(320·populated axe·repair loop), 사용자가 시각 방향 선택)** → R3(토큰)·R4(컴포넌트) → R5(DESIGN.md 저장) → R6(DESIGN.md 토큰의 스택 테마 배선 + 네이티브 쇼케이스 확인 + 게이트 — ADR-058#amend-4). **사용자가 R2 concept 방향을 선택하고 R6 쇼케이스를 승인한 뒤** concept 시안을 삭제하고(쇼케이스·배선은 커밋) `/plan-milestone`으로 진행 권장한다(M/F가 아직 없으면 — ADR-057; `contract-ready` M에 task 0건/`draft`가 있으면 `/plan-workitem M<N>` → `/seal-milestone M<N>`(ADR-060); 이미 구현 중이면 해당 task workflow 또는 다음 M). DESIGN.md *내용*·인터페이스 할당 SSOT는 ADR-073.
- ARCH `## 7-1`/`## 7-2`/`## 7-3`/`## 7-4` / `## 7-5` 의 채움/삭제/cross-reference 정책은 [ADR-073](../90-decisions/boilerplate/ADR-073-interface-and-design-content-v2.md) D1 (ADR-073 D8 cross-surface enforcement 포함) SSOT.

## 3. 작업 단위 분해
- 마일스톤·feature 문서는 첫 마일스톤(M1)부터 `/plan-milestone`이 만든다(ADR-057 — bootstrap-project는 charter/architecture까지). UI 마일스톤은 `/design-milestone M<N>`이 화면 층(브리프·코드 프로토타입·스냅샷·PX)을 확정해 `contract-ready`로 올린다(ADR-072).
...
- (UI 마일스톤) `/design-milestone M<N>`이 화면 경험 계약(매니페스트·승인 스냅샷·코드 프로토타입 — `docs/20-system/prototypes/M<N>/`)을 확정한 뒤 task 분해로 진행한다. UI 확정 feature는 매니페스트 참조(또는 면제 기록) 없이 `/plan-workitem` 분해가 차단된다 (ADR-072 D9).
...
```
discover → bootstrap → plan-milestone → (UI) design-milestone → [M/F = contract-ready]
...
| 전이 | 최소 조건 |
|------|-----------|
| (M/F) draft → contract-ready | 비-UI: plan-milestone 라운드 완료 / UI: design-milestone R7 완료(매니페스트·스냅샷·PX) + 확정 재대조 통과 + 원장의 이 M 영향 및 `(미할당)` `open` 0건. **잠금 아님** — task 분해 중 계약 수정 가능 |
