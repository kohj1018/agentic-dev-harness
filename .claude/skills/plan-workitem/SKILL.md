...
3-I. **계측 line item authoring (ADR-042#amend-2 결정 3 / ADR-062)**: 담당 feature 문서 `## 8-1` 의 **계측 필드**(`<이벤트명> @ <발생 지점> / 속성: <목록> / 도구: <도구>`)가 채워져 있으면 그 이벤트를 심는 작업을 **담당 task 의 `## 3` line item 으로 옮긴다** — 이벤트명·발생 지점·속성 목록·도구를 그대로 적고, *어느 파일의 어느 지점에* 심는지까지 위 3-G 형식으로 쓴다. **feature 문서에만 남기고 task 로 옮기지 않으면 그 이벤트는 구현되지 않으며, 데이터는 소급 수집이 불가능하다.**
   ...
   - **승인 UI 로 배선 가능한 계측만 authoring 한다 (ADR-072 D5 정합 — dogfood Round 11 발견)**: 그 feature 가 승인 프로토타입을 쓰면(`## 7` 에 `프로토타입:` 참조), 각 이벤트가 **그 화면의 콜백 표면에서 실제로 발화 가능한지** 매니페스트 `source[]` 의 컴포넌트 시그니처로 대조한다. 프로토타입이 내부에서 삼키는 상호작용(예: 공백 제출을 자체 거부하고 콜백을 부르지 않는 입력 컴포넌트)은 **배선 지점이 없어** 그 이벤트를 심을 수 없다 — 그런 이벤트는 line item 으로 만들지 말고 «남은 미결정 사항» 에 `- 계측 배선 불가: <이벤트> — 승인 UI `<screen>` 에 콜백 없음(해소하려면 `/design-milestone` 재진입)` 으로 surface 한다. 억지로 line item 을 박으면 구현이 도달 불가능한 dead code 를 만들거나 승인 UI 를 무단 변경한다(실측: 4개 격리 구현 중 2개가 dead code 를 넣었다).
