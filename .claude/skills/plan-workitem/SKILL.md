---
name: plan-workitem
description: 입력 마일스톤의 전 feature를 task로 분해하고 전 AC·매핑을 1회 완성 (milestone·feature 문서 *생성*은 plan-milestone 담당 — ADR-050/ADR-075). Claude Code plan 모드와 다름 — task 분해기. `M<N>` 입력 시 마일스톤 전체 계획 스냅샷(ADR-057#amend-3).
...
- **`M<N>` 입력 — 마일스톤 전체 계획 스냅샷 (ADR-057#amend-3)**: 본 마일스톤 `## 3. 포함되는 기능`의 모든 feature를 **한 번의 실행으로** 완성한다. 내부적으로 feature별 순차 authoring은 허용하되 사용자에게 feature 단위 재호출을 요구하지 않는다.
  ...
  - `## 3` 단계별 가이드는 **모든 feature**에 대해 3-G로 완성한다. AC 해석 확정(9-1)·경험 계약 입구 점검(ADR-072 D9)은 전 feature 수행.
...
**확정 후 변경 경계 (ADR-060 D6 / ADR-072 D3·D9)**: plan-workitem은 마일스톤 전체 계획 스냅샷의 작성과 멱등 재개만 제공한다. **M이 `contract-ready`인 동안에는 상위 계약(프로토타입·FAC·scope)도 아직 잠기지 않았다** — 분해 중 상위 계약 결함이 드러나면 다음 마일스톤으로 보내지 말고 사용자에게 보고해 `/plan-milestone M<N>` 재개 또는 `/repair-plan M<N>`으로 그 자리에서 고친다. 잠금은 `/seal-milestone`이 `ready`를 부여한 뒤에 발생하며, 그 뒤의 기획 변경은 새 마일스톤(M<N+1>)이다. 구현 중 계획과 *근본* 충돌이 드러나면 자동 재계획하지 않고 **사용자에게 중단·보고**(§4.5b amend-3 결정 3)하며, 일반 오류(테스트·타입·프로토타입 불일치)는 repair한다. 미완 R5는 같은 `/plan-milestone M<N>` 재실행으로 이어간다.

반드시 먼저 읽을 파일:
- `docs/10-charter/PROJECT_CHARTER.md`
- `docs/20-system/ARCHITECTURE_OVERVIEW.md` — *해당 스택 한정 sub-section 만*: `## 7-1` (API 프로젝트), `## 7-2` (CLI), `## 7-3` (백엔드), `## 7-4` (프론트), `## 7-5` (모바일 클라이언트 — ADR-073 D1). 비해당 sub-section 은 회수 X (ADR-019 minimal 정합).
- `docs/20-system/DESIGN.md` — *UI 프로젝트 한정*. UI 판정은 **ADR-073 D9 "UI 판정 다중신호 절차"** 적용(부재→비-UI / status≠draft→UI / status=draft→추가신호). UI 확정 시 본문 회수 + cross-check 활성, 비-UI/skip 시 사유 echo.
...
3-I. **계측 line item authoring (ADR-042#amend-2 결정 3 / ADR-062)**: 담당 feature 문서 `## 8-1` 의 **계측 필드**(`<이벤트명> @ <발생 지점> / 속성: <목록> / 도구: <도구>`)가 채워져 있으면 그 이벤트를 심는 작업을 **담당 task 의 `## 3` line item 으로 옮긴다** — 이벤트명·발생 지점·속성 목록·도구를 그대로 적고, *어느 파일의 어느 지점에* 심는지까지 위 3-G 형식으로 쓴다. **feature 문서에만 남기고 task 로 옮기지 않으면 그 이벤트는 구현되지 않으며, 데이터는 소급 수집이 불가능하다.**
   ...
   - **승인 UI 로 배선 가능한 계측만 authoring 한다** — 판정 규칙은 아래 3-S (b) 가 SSOT 다.
   - 계측 도구가 미설치면 **authoring 만** 한다 — 설치는 `/implement-workitem` 이다(ADR-040#amend-1 / ADR-052 install-ownership 3분할).
3-P. **승인 프로토타입 참조 + PX↔AC 매핑 + 전환 흐름 authoring (ADR-072 D9·D3·D1 — 이중 잠금 2/2)**:
   ...
   - **PX↔AC 매핑 (ADR-072 D3·D9)**: feature `## 7`의 `경험 결정(PX):` 인벤토리 각 PX를 그것을 구현하는 AC로 매핑해 feature `## 7-3. 프로토타입 경험(PX) ↔ AC 매핑`에 `PX-M<N>-<screen>-NN → T-NNN:AC-M`으로 기입한다(해당 AC 본문에 `(PX-M<N>-<screen>-NN)` 태그 가능). 어떤 AC도 참조하지 않는 PX(unmapped PX)는 "남은 미결정 사항"에 `- unmapped PX: <PX-M<N>-<screen>-NN> — 커버 task/AC 없음`으로 surface(unmapped FAC 패턴과 동형 — [Plan-FAC-coverage]가 재점검). PX 원천은 매니페스트 `px[]`와 코드 주석이다(HTML 아님).
   - **전환 흐름 소비 (ADR-072 D1·D9 — `## 9` 전환 표 존재 시)**: 마일스톤 `## 9. 화면 전환`에서 이 feature가 owner인 행을 회수해, 그 **존재하는 각 path type 행(primary·failure·recovery)**이 task AC로 커버되는지 확인한다(FEATURE §8-1 복구 흐름과 정합). 미커버 path는 "남은 미결정 사항"에 surface. `## 9`가 "(해당 없음)"이면 skip.
...
3-S. **승인 표면 대조 self-check (분해 직후 1회 — ADR-072#amend-2)**:
   위 3-G·3-I·3-P·3-W 로 authoring 이 끝난 뒤, 각 task 를 **승인된 표면**(프로토타입 매니페스트 / DESIGN `## 7` 인벤토리 / feature `## 7` FAC)과 3축으로 대조한다. 셋 다 *plan 이 승인 표면을 보지 않고 `## 3` 를 쓰는* 동일 원인에서 나온 결함을 막는다(dogfood Round 11 — 같은 계열 4건). 자동 차단 X — 위반은 전부 "남은 미결정 사항" surface.
   - **(a) UI 지시의 출처 (ADR-073 D8 / ADR-072 D9)**: `## 3` 의 각 단계가 지시하는 **사용자가 보게 될 요소**(새 컴포넌트·배너·빈 상태·오류 문구·새 상태 표시)마다 출처가 다음 중 하나여야 한다 — ① 승인 매니페스트 `source[]`·`px[]` 에 실재(→ 3-P 재사용 line item 으로 처리) ② DESIGN `## 7` 인벤토리의 기존 컴포넌트 재사용 ③ **신규**. ③이면 그 task `## 3` 에 `+ DESIGN.md ## 7. Components 에 한 줄 등록 (그 category의 expected 상태 설계 포함)` line item 을 같이 박고(위 «신규 인터페이스 요소» 형식), 그 요소가 **승인 화면의 시각 표면을 바꾸면** 등록만으로 끝내지 말고 `- 승인 UI 변경 필요: <요소> @ <screen> — 재승인 경로 `/design-milestone M<N>` 재진입 또는 다음 M(ADR-060 D6)` 을 "남은 미결정 사항" 에 올린다. **출처 ①~③ 어디에도 못 넣는 UI 지시는 `## 3` 에 쓰지 않는다.** 또 그 요소를 요구하는 AC 가 0개면 `- UI 지시에 대응 AC 없음: T-NNN `## 3` step <N> — <요소>` 를 같이 surface 한다(AC 없는 UI 지시는 검증도 수용도 되지 않는다). 근거: 등록 *결정* 의 출발점이 plan 인데 plan 이 빠뜨리면 validate 축 5 가 `P1 [Design-inventory-planless]` 로 잡더라도 **이미 구현이 끝난 뒤이고 비차단 기록 등급**이다.
   - **(b) 계측의 배선 가능성 (ADR-072 D5)**: 3-I 로 옮긴 각 계측 이벤트가 **승인 화면의 콜백 표면에서 실제로 발화 가능한지** 매니페스트 `source[]` 의 컴포넌트 시그니처로 대조한다. 프로토타입이 내부에서 삼키는 상호작용(예: 공백 제출을 자체 거부하고 콜백을 부르지 않는 입력 컴포넌트)은 **배선 지점이 없다** — 그런 이벤트는 line item 으로 만들지 말고 `- 계측 배선 불가: <이벤트> — 승인 UI <screen> 에 콜백 없음(해소하려면 `/design-milestone` 재진입)` 으로 surface 한다. 억지로 박으면 구현이 도달 불가능한 dead code 를 만들거나(실측: 4개 격리 구현 중 2개) 승인 UI 를 무단 변경한다. **심벌 실재까지 본다 (2026-09-12 확장)**: 계측 이벤트뿐 아니라 **`## 3` 가 이름으로 지목한 코드 심벌 전부**(생성자 인자·enum 값·메서드·prop)를 같은 대조에서 확인한다 — 이미 승인 컴포넌트의 시그니처를 열고 있으므로 추가 비용이 거의 없다. 없는 심벌을 지목하면 그 line item 을 고치고, 고칠 수 없으면 `- 심벌 부재: <심벌> — <어디서 사라졌는가>` 로 surface 한다. **슬롯 유무까지 본다**: 어떤 PX·AC 가 승인 컴포넌트에 **배선점**(focus node·controller·scroll controller·key 등)을 요구하는데 그 컴포넌트가 그것을 노출하지 않으면, **배선 task 로는 달성할 수 없다.** 그 경우 `- 배선점 부재: <무엇> — <어느 승인 컴포넌트가 무엇을 노출하지 않는가>` 로 surface 하고, 재승인 경로(`/design-milestone` 재진입 또는 다음 M)를 함께 적는다. **「배선만」 계약(ADR-072 D5-3)은 승인 UI 가 배선점을 가질 때만 성립한다.** 실측(Round 12): `PX-M1-add-habit-01` 의 「닫히면 열기 전 위치로 돌아온다」를 배선하려면 `TodayListScreen` 이 FAB 에 `focusNode` 슬롯을 내야 하는데 없다 — 계획은 「`FocusNode` 를 보관하고 `requestFocus()` 하라」고 지시했고, 구현은 **어디에도 붙지 않는 노드**를 만들었으며, AC 테스트는 **자체 하네스 버튼**으로 통과했다. AC 는 충족인데 PX 는 제품에서 미검증인 상태가 봉인을 통과했다. 실측(Round 12): T-002 `## 3` 이 `addButtonLayout: AddButtonLayout.extendedFab` 를 넘기라고 썼는데 **그 enum 은 같은 날 앞선 커밋에서 삭제됐다**(A/B 확정으로 탈락안 제거). 「A안 확정」을 전달받고도 «A안 = 그 enum 값» 이라는 이전 판독을 그대로 옮긴 것이다. 3-S (a)는 *가시 요소의 출처*를, `/validate-plan` 축 9·10 은 *DESIGN·ARCH 결정*을 보므로 셋 다 이것을 못 본다.
   - **(d) DESIGN 확정 결정과의 충돌 (ADR-073#amend-1 결정 4)**: task `## 3` 의 지시가 **DESIGN 이 이미 확정한 결정과 반대 방향인지** 본다. 첫 검사 대상은 **`## 3` 글꼴** — 스캐폴드가 심은 패밀리(`Geist`·`Inter` 등)를 「유지한다」·「옵션을 추가한다」로 지시하고 있으면 위반이다. 같은 요령으로 `## 2` 색·`## 4` 레이아웃 수치도 본다. 위반이면 그 line item 을 **DESIGN 결정 쪽으로 고쳐 쓰고**, 고칠 수 없으면 `- DESIGN 충돌: <무엇> vs DESIGN `## N` <결정>` 으로 surface 한다. 실측(Round 12): T-005 `## 3` step 3 이 「`Geist`/`Geist_Mono` 에 `preload: true` 명시」라고 **DESIGN 이 기각한 패밀리를 유지하라고** 지시하고 있었고, 그 자리에 「self-host 폰트가 첫 렌더 전에 적재되도록」이라는 **맞는 말이 붙어 있어 더 잡기 어려웠다**.
   - **(c) FAC 매핑 행의 증명 문장 (ADR-037 / ADR-060 D7)**: feature `## 7-1` 의 **매핑 행마다 증명 문장 1줄**을 같은 줄에 붙인다 — 형식 `- FAC-N → T-NNN:AC-M — 증명: AC-M 의 <어느 조건>이 FAC-N 의 <어느 요구>를 검증한다`. 증명 문장을 **쓸 수 없으면 그 매핑은 비어 있는 것**이므로(우변은 실재하나 의미가 다름 — 커버리지 검사는 우변 실재만 본다) 행을 그대로 두지 말고 ① 그 AC 문안을 FAC 를 검증하도록 고치거나 ② 그 FAC 를 검증하는 AC 를 추가한 뒤 다시 쓴다. 둘 다 이 마일스톤에서 불가능하면 `- FAC 증명 불가: FAC-N → T-NNN:AC-M — <무엇이 빠졌는지>` 를 "남은 미결정 사항" 에 올린다(unmapped 와 달리 봉인을 자동 차단하지는 않는다 — 사용자 결정). PX↔AC(`## 7-3`)에는 적용하지 않는다. **두 슬롯을 다 채운다** — 「AC-M 의 <조건>이 FAC-N 을 **그대로** 검증한다」처럼 오른쪽 슬롯을 「그대로·동일하게·바로」로 때운 행은 증명이 아니다(실측 Round 12: 14행 중 3행). 코드를 인용할 때는 **줄 번호가 아니라 식별자**를 쓴다 — 줄 번호는 다음 커밋에 어긋난다. **제3 형식 (ADR-037#amend-4)**: 승인 표면이 이미 검증하는 FAC 는 `- FAC-N → 승인 스냅샷 <경로> (manifest: <screen id>) — 증명: …` 로 적고 unmapped 로 세지 않는다 — 단 (i) 그 산출물이 경로로 실재하고 (ii) 이번 M 의 어느 task 도 그 화면 `source[]` 파일을 변경 대상에 넣지 않을 때만이다.
   - **(e) AC당 modality 하나 (ADR-065 D1)**: 한 AC에 «텍스트는 자동 테스트, 여백·위계는 사용자 관측»처럼 두 modality가 필요하면 **AC를 둘로 쪼갠다**(AC-1a/AC-1b). `## 6-1`에 같은 AC의 행이 둘이면 위반이다.
4. 관련 문서 링크를 함께 기록한다.
...
**경험 좁힘 무조건 질문 (ADR-072 D5)**: 해석 후보 중 어느 쪽을 골라도 무방한 *내부 엔지니어링 선택*은 지금처럼 자율 확정한다. 그러나 해석이 **사용자가 보고 느낄 것(보이는 것·눌렀을 때 일어나는 일·쓰여 있는 말)을 프로토타입·상위 약속보다 좁히는 경우**는 권장 선택을 확정하지 말고 "남은 미결정 사항"에 질문으로 올린다 — 질문 피로 방지선은 이 비대칭이 담당한다.
...
## 정합성 self-check (분해 직후 1회 실행, ADR-026#amend-1 + ADR-073 D8)
...
> Prefilter 한계 명시: 본 키워드 매칭은 *best-effort*. false positive/negative 가능 — prefilter 가 놓친 task 는 **validate-workitem (validator) 의 CHECK 단계가 catch** (2-layer defense — plan prefilter 가 1차, validator 가 2차). *implement/builder 가 catch 하지 않는다* — implement 는 EXECUTE 전용 (ADR-073 D8 책임 분배).
...
- **상태 점검은 *task 의 use-case 해당 상태* 한정** (DESIGN.md `## 7` 의 *전체* category expected 상태 설계(ADR-073 D2 — interactive/data/static)는 별도 — reviewer Design Consistency `[Design-state]` 책임). 본 self-check 는 *task 본문이 명시한 상호작용* (예: hover/disabled 가 use-case 에 등장하는데 AC 에서 언급 누락) 만 점검. 누락 상태가 있으면 "남은 미결정 사항" 에 `- use-case 상태 누락: T-NNN — <상태> 가 task 본문에 등장하지만 AC 미언급` 명시. 자동 차단 X.
- task 본문·AC에 박힌 사용자 표면 문구가 DESIGN.md `## 10` Voice & Writing(어조·용어 번역표)과 정합하는가? placeholder 카피·내부용어 노출 발견 시 "남은 미결정 사항"에 `- voice 위반 의심: T-NNN — <문구>. DESIGN.md ## 10 정합 권장` 명시 (ADR-073 D6).
...
- 예: `- 신규 IconButton 컴포넌트 생성 + DESIGN.md ## 7. Components 에 한 줄 등록 (그 category의 expected 상태 설계 포함 — ADR-073 D2)`
- 예: `- 신규 error code USER_LOCKED 도입 + ARCH ## 7-1 error 레지스트리 등록`
- 예: `- 신규 CLI 출력 모드 --json 추가 + ARCH ## 7-2 출력 포맷 등록`

이로써 등록 *결정* 은 plan 이 authoring 하고, builder 는 task 스펙을 *기계적으로 실행* — 등록 책임이 executor 의 독립 판단에 박히지 않는다 (ADR-073 D8 책임 분배 / ADR-005 정합). validator 는 본 line item 이 실행됐는지 점검 (`/validate-workitem` + `validator.md` CHECK 단계).
...
5. **ARCHITECTURE_OVERVIEW.md `## 7-1`/`## 7-2`/`## 7-3`/`## 7-4` 의 *기존 결정* 변경 또는 신규 항목 추가 의심** (예: API versioning 정책 변경 / 인증 방식 변경 / 라우팅 전략 변경). 인터페이스 결정 책임 분배 (ADR-073 D1) 정합.
...
**분할 authoring 순서 (ADR-074 D10·D9-2)** — 마일스톤 **전체 스냅샷 원칙은 불변**이다(ADR-057#amend-3). 한 실행 안에서 세 단계로 간다.
...
조각이 상한에 닿으면 **부분 보고는 오지 않는다** — 상한은 마무리 턴 없이 작업 중간에서 자르고 에이전트에게는 상한 접근 신호가 없다(ADR-074 D9). 그러므로 **메인이 워킹트리를 직접 읽어 「이미 쓴 파일 목록」을 만들고 그 목록을 첫 회수 dispatch 에 실어** 보낸다 — 재개한 에이전트가 같은 파일을 다시 열지 않는다(ADR-074 D10). 실측(Round 12): 11 산출물을 한 dispatch 에 넣었더니 `planner` 가 **2회 연속 보고 0건**으로 멈췄고 3회차에야 끝나 누계 463K 토큰이 들었다.
