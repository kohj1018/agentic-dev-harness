...
3. `docs/40-validation/IMPROVEMENT_GUIDE.md`의 본 milestone sub-section(`### M-N` 그룹)에서 `status: open` 항목을 회수한다 — `## 2. 열린 항목` 안의 그룹. **`### M-N — instruction improvement 후보` 그룹은 회수·4-판정 대상이 아니다** — 대상이 이 프로젝트의 코드가 아니라 boilerplate harness 문구·검사 장치라 본 skill 의 책임 경계 밖이다. 그 그룹은 `status: open` 으로 두고 출력에 «instruction improvement N건 — boilerplate 저장소 소관, 본 라운드 대상 아님» 한 줄만 남긴다(D3 의 «판정 없이 open 두지 않는다» 는 *프로젝트 결함·개선* 항목에 건다). **`## 5. Repair decision log`는 회수 대상 아님** (closed records — 이미 지나간 판단).
...
2-D. **승인 컴포넌트 행동 변경 고지 (ADR-072 D5-3-1)**: 수정이 승인 프로토타입 파일의 **표현이 아닌 행동**(이벤트 핸들러·포커스 관리·가드 로직)을 바꿨으면 — 그 변경이 브리프 `## 인터랙션 계약` 의 문장을 **바꾸면 재승인 대상**이라 본 skill 이 고치지 않고 `- 판정: Adopt — blocked: 재승인` 으로 남긴다(ADR-070#amend-1 결정 1). **바꾸지 않으면** 고치되 수행 4 의 `## 5` 항목에 `- 승인 컴포넌트 행동 변경: <무엇> — 인터랙션 계약 무변경` 한 줄을 적는다. `[Design-reuse-drift]` 는 표현 기준이라 여기에 발화하지 않는다 — 그래서 이 고지가 유일한 추적 경로다.
...
2-V. **자체 검증 — 즉시 파손 감지 (ADR-068 D6 검증 집합)**: 위 2·2-S·2-P·2-R·2-E·2-B를 마친 뒤 1회 수행한다. 내용은 다섯이다: (i) 본 라운드에 추가한 회귀 테스트가 전부 Green이고 **그 매체의 졸업 검사에 묶여 있는지** 확인한다 — 단위·통합 테스트는 통합 `validate`(졸업 item 2), **브라우저·디바이스 렌더가 있어야 재현되는 회귀는 `validate:e2e`**(졸업 item 3 E2E hard-block)다. 후자를 `validate` 에 넣으려 하지 마라 — 좌표·폭·포커스처럼 렌더 엔진이 있어야 성립하는 증상은 jsdom 매체가 없어서 «묶이지 않았다»가 아니라 «다른 게이트가 검사한다»가 맞다(dogfood Round 11 실측 — `M1-001`·`M1-005`), (ii) 변경 파일과 교차하는 task의 `## 6-1` 자동 테스트 매핑 대상을 실행한다, (iii) 외부 경계·핵심 journey를 건드렸으면 해당 integration/e2e smoke를 실행한다, (iv) 통합 검증 명령이 `--changed`를 지원하면 `validate --changed`를(미지원이면 통합 `validate`를) 1회 실행한다.
...
5. **원본 finding 종결 — 4-판정 전부 (ADR-070 D3)**: IMPROVEMENT_GUIDE `### M-N`·QA_FINDINGS `## M-N`의 원본 항목에 대해 — Adopt/Adopt-modified는 **원본 `- 재현:` 절차를 다시 실행해 통과를 관측한 뒤** `status: resolved` + 하위 줄 `- 재현 재실행: <날짜> 통과`; Reject-FP는 `status: resolved` + `decision: rejected-fp` + 근거; Reject-context는 `status: resolved` + `decision: rejected-context` + 근거; needs-confirmation 유지분은 `status: open` 그대로 + 막힌 이유 갱신(판정된 open); **`Adopt — blocked:` 항목도 같다**(ADR-070#amend-1 결정 1 — `status: open` + 그 하위 줄이 판정 기록이다). 재현 줄이 없는 항목(P1·P2·개선·문서)은 `- 종결 근거: <검증 수단 → 결과>` 한 줄이 종결 증거다. **판정 없이 원본을 `open`인 채 두지 않는다**(Reject 원본이 열려 있으면 다음 stabilize가 같은 P0를 다시 센다). closed records인 `## 5`로 옮기지 않고 *status만* 토글한다.
