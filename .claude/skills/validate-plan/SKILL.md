...
**slice 기준 (ADR-004#amend-9 결정 4)**: 회수 문서가 **10개를 넘으면 축·범위를 나눠 dispatch 한다.** 실측(Round 12 `M1`): charter·원장·ARCH·DESIGN·M·feature 3·task 5·매니페스트 + `source[]` PX grep = 15+ 문서를 한 dispatch 에 넣었더니 reviewer 가 **리뷰 파일을 쓰기 직전에** 상한에 닿아 산출물 0건으로 멈췄다. 회수 1회로 완주했으나 누계 356K 토큰이 들었다.
...
- **[Plan-FAC-coverage] 반전**: `## 7-1`(FAC) *및 `## 7-3`(PX)* 빈 shell은 *정상* — task 0건이면 unmapped FAC·**unmapped PX**를 P0로 올리지 **않는다**(design-milestone R7이 PX 인벤토리만 채우고 plan-workitem 전이라 PX↔AC 매핑이 비어 있는 게 정상 — ADR-072 D3). shell이 *형식적으로 깨졌을 때만* P2. **[Plan-design] recovery-path 유예(F9와 동형, ADR-072 D1)**: task 0건이면 `## 9` 전환 표의 존재하는 각 path type 행(primary/failure/recovery)이 매니페스트 화면·브리프에 나타나는지만 보고, AC 매핑 미비는 P0/P1로 올리지 않는다(plan-workitem 후 정상 모드에서 AC 커버 재점검).
- **활성**: milestone-plan 4차원(아래).
- 혼합 마일스톤은 **feature 단위로** mode 적용. task가 1건+면 12차원.

검토 차원 (12 dimensions — reviewer.md의 *Plan Quality 12 차원* 정합 — ADR-073 D8 / ADR-060 D8):
