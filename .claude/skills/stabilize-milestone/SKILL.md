...
3. **FAC ↔ AC unmapped 검출** ([ADR-037](../../../docs/90-decisions/boilerplate/ADR-037-spec-coverage-audit.md)#amend-1 영속 SSOT `## 7-1` 정합):
   - 본 마일스톤의 모든 feature 문서 `## 7-1. FAC ↔ AC 매핑표`에서 *unmapped* 또는 *비어 있음* 항목 회수. **우변이 `승인 스냅샷 <경로> (manifest: …)` 형식인 행은 unmapped 가 아니다**(ADR-037#amend-4 — 승인 표면이 검증자다). 우변이 비었거나 `(해당 task 없음)` 처럼 **검증자를 지목하지 않은** 행만 센다. `## 7-3` PX↔AC 도 같다(결정 1-b).
   - 발견 시 **QA_FINDINGS.md 본 마일스톤 `### P0`**에 `P0 [Spec-gap] F-NNN:FAC-N → unmapped`(`- 출처: preflight`)로 등재한다(ADR-073#amend-2 결정 4 / ADR-070 D7 — 계약 결함은 QA 원장, 그래야 졸업 item 5가 센다) + graduation NO 유지 + 사용자 보고. 구현 후에는 plan-workitem으로 task를 자동 추가하지 않으며, 담당 task가 있으면 repair, 없으면 ADR-057#amend-3 결정 6의 사용자 명시 결정 경로를 따른다.
...
5. **DESIGN.md + ARCH 7-x cross-surface drift 검출** (ADR-073 D8) — *(5-2) 는 정규식 기반 deterministic, (5-3)(5-4) 는 mechanical/best-effort heuristic — 휴리스틱 한계는 항목별 echo*:
   ...
   - **(d) 4차 — 폐쇄 후 수리 기록**: `IMPROVEMENT_GUIDE.md ## 5`의 본 마일스톤 항목 `files:` 목록(ADR-073#amend-2 결정 3 — 마일스톤 층 수리가 바꾼 파일은 여기에만 있다).
   - **(e) 모두 실패 시**: 본 task 는 *조사 불가* 로 표시 + IMPROVEMENT_GUIDE 에 `P2 [Stabilize-recovery] T-NNN — 변경 파일 회수 불가` 기록 후 다음 task 로 계속.
   - **(f) 회수 결과 echo (ADR-073#amend-2 결정 3)**: `회수 파일 N개 ((a) x · (b) y · (c) z · (d) w)`. N = 0이면 5-2·5-2b·5-3을 실행하지 않고 `P2 [Stabilize-scope-empty] 회수 파일 0건 — 5-x 미실행`을 기록한다. 5-2·5-2b·5-3 출력은 `대상 K파일 / 위반 M건`으로 둘을 함께 적는다.
   ...
   - **Dart 계열** — 확장자 `.dart`: ① `Color\(0x[0-9A-Fa-f]{6,8}\)` 및 `Color\.from(ARGB|RGBO)\(` → `P1 [Design-rawhex] <file:line>` ② `\b(Colors|CupertinoColors)\.(?!transparent\b)[a-zA-Z]+` (프레임워크 기본 팔레트 사용 — 값을 박은 것은 아니나 프로젝트 토큰을 우회함). **`Colors.transparent` 는 제외한다** — 그것은 색이 아니라 «색을 칠하지 않는다»는 뜻이고 어느 디자인 시스템에서도 토큰화 대상이 아니다(dogfood Round 11 실측 2건 오탐 — 발견 60). 한 낱말 예외라 미탐 위험이 거의 없고, 매번 브리프 `## 14` 에 사유 주석을 쓰게 만드는 비용을 없앤다 → `P1 [Design-token-grep] <file:line> — 테마에서 가져오도록 교체 권장`. 두 라벨은 원인과 수정 방법이 달라 분리한다. **`[Design-token]`(접미 없음)은 reviewer 의 LLM 판정 차원이 이미 쓰는 라벨이므로 grep 결정분에는 `-grep` 접미를 붙인다** — `[Design-voice-grep]`/`[Design-voice]` 분리와 같은 규칙이며, 회귀 신호 집계가 라벨 정확 일치로 동작하므로 겹치면 오집계된다.
   - **Dart 정의 라인 예외**: `static +const +(Color +)?[A-Za-z_][A-Za-z0-9_]* *= *Color\(` 형태의 줄은 토큰 *정의*이므로 제외한다(웹의 CSS custom property 정의 라인 예외와 동형). **타입 표기는 선택이다** — `static const paper50 = Color(0xFF…)` 처럼 중복 타입을 생략하는 것이 Dart 관례이고 `dart format`·lint 가 그쪽을 민다. 예전 패턴(`static +const +Color +…`)은 **타입을 적은 줄만** 잡아 토큰 정의 파일 전건을 오탐했다(dogfood Round 12 실측 — 발견 77).
   - **`_theme` 매니페스트 예외 (발견 77)**: `docs/20-system/prototypes/_theme/manifest.json` 의 `source[]` 에 등록된 파일은 **토큰 정의·표시 표면**이므로 본 검사 대상이 아니다. R6-1 이 만드는 쇼케이스(`theme_gallery.dart`·`Theme.stories.tsx`)는 **토큰 값을 라벨로 보여 주는 것이 존재 이유**라 hex 문자열이 정상이고, 매 라운드 같은 오탐을 낸다. **파일명 휴리스틱이 아니라 매니페스트를 근거로 삼는다** — 「파일명으로 파일 전체를 빼지 않는다」는 원칙과 충돌하지 않고, 제품 코드의 사용처 위반은 그대로 잡힌다(그 파일들은 `source[]` 에 없다).
   - **자가 검사 fixture 예외 (발견 77 ③)**: design gate 자가 검사 fixture 는 **색을 의도적으로 박아 둔 입력**이다(known-bad 의 2:1 대비 쌍이 그 파일의 존재 이유다). Dart 는 `@Tags(['design-gate'])` 가 붙은 파일, 웹은 게이트 어댑터 안의 인라인 HTML 이므로 `apps/**` 밖이다. **태그를 근거로 삼는다** — 그 태그는 이미 `--exclude-tags=design-gate` 로 통합 validate 에서 그 파일들을 빼는 데 쓰이고 있어 새 표식을 만들지 않는다.
   ...
   5-2b. **UI 프로젝트 — voice grep** (정규식 deterministic — ADR-073 D6): 5-0 회수 변경 파일 중 **UI surface 파일 집합**(ADR-073#amend-2 결정 1)의 파일에서, 각 줄의 **렌더되는 텍스트 구간** — 따옴표 안 문자열(`"…"`·`'…'`·`` `…` ``) **과 JSX·HTML 텍스트 노드**(`>`와 다음 `<` 사이) — 에만 (a) placeholder 카피 패턴(`lorem ipsum`/`TODO copy`/`sample text`/`여기에 텍스트`), (b) DESIGN.md `## 10` "금지 표현"의 `[grep 가능]` 정규식을 적용한다(ADR-073#amend-2 결정 2 — 식별자·키 이름은 대상이 아니다). 일치 시 `P1 [Design-voice-grep] <file:line> — DESIGN.md ## 10 위반`(기록 등급 — 차단 아님) 기록. DESIGN.md `## 10` 부재 시 (a)만 수행. **DESIGN.md 자체는 grep 대상 제외** (규칙 정의 영역 — 5-2와 동형). 출력은 `대상 K파일 / 위반 M건`.
...
8. **검증 장치 노후 감지 `[Guard-drift]` (deterministic — ADR-063 D4)**: **침묵 우선 — 아래 (a)~(e) 가 전부 정상이면 출력에 한 줄도 남기지 않는다** (skip 사유 echo 도 하지 않는다. 정상 상태를 매번 보고하면 그것이 노이즈이고, 검증 장치가 매 마일스톤 변경되는 것은 정상이 아니다). **단 아래 "선행: 파일 부재 처리" 는 예외다** — 점검을 아예 수행하지 못했다는 사실은 침묵하면 안 된다.
   ...
   - (b) **design gate copied-from 4방향 판정 (ADR-063#amend-1)** — `status: ready`인 경우에만 세 값을 읽는다: canonical sha C(`.claude/skills/stack-guard/assets/design-gate.mjs`), registry `copied-from` R, 사본 sha P(`adapter path`). (i) P = R, C ≠ R → `P2 [Guard-drift] canonical 갱신됨 — /stack-guard 재실행 권장(무수정 사본이라 교체된다)` (ii) P ≠ R, C = R → `P2 [Guard-drift] 사본 로컬 수정됨 — 재실행해도 덮어쓰지 않음(diff 보고). 수정분을 canonical로 역류할지 검토: IMPROVEMENT_GUIDE [ADR-candidate]` (iii) P = C ≠ R → `P2 [Guard-drift] registry만 낡음(코드 동일) — copied-from 갱신 권장(/stack-guard 재실행이 갱신)` (iv) P ≠ R, C ≠ R, P ≠ C → 둘 다 + `사용자 결정 필요`. 전부 읽기 전용 — mtime·날짜 비교 없음; sha256 도구 부재 시 skip + 사유 echo.
...
3-P. **(옵션) 탐색적 QA via browser/E2E MCP** (ADR-048#d6 registry-driven / ADR-043 보안 — STACK_SETUP_PLAN `## Optional MCP Connectors`에 browser/E2E capability MCP가 *등재 + `agent access` 부여* + UI 프로젝트일 때만; 미등재·access 미부여·비-UI는 silent skip + 사유 echo): 실제 앱을 구동해 본 마일스톤 feature의 시나리오(happy/alt/fail) + qa 엣지케이스를 *탐색*한다(accessibility 트리·클릭/입력·스크린샷·네트워크). 발견한 결함을 `docs/40-validation/QA_FINDINGS.md`에 기록하고, **재현 케이스를 영속 E2E 테스트(`validate:e2e`에 묶이는 커밋 가능한 파일)로 남길 것을 권장**(자동 커밋 X — stabilize는 코드·커밋 금지, 후속 task 제안). 실패는 `Type: bugfix` task(ADR-039)로 라우팅. **보안: `browser_run_code_unsafe`류 RCE급 도구는 사용하지 않는다** — accessibility snapshot·표준 브라우저 조작만. **본 단계는 메인 세션이 직접 수행한다** — 탐색 중 임시 스크립트가 필요하면 메인이 scratch 경로에 두고 경로를 넘긴다(ADR-050#amend-2 결정 2 — 프로젝트 트리에 만들지 않는다).
3-V. **경험 게이트 — 구현 화면 vs 승인 스냅샷 대조 (ADR-072 D7, UI 확정 마일스톤 한정)**: MCP 불요 체계 감사. **실행 자체는 의무 — silent skip 금지**(미실행 사유 echo; 판정은 report-only). **본 단계도 메인 세션이 직접 수행한다** — 렌더·대조에 쓰는 임시 스크립트는 메인이 scratch 경로에 두고 경로를 넘긴다(ADR-050#amend-2 결정 2).
...
5-D. **등재 전 dedup (필수 — ADR-070#amend-1 결정 2)**: 6-S 검토를 통과한 각 P0·P1 을 원장에 적기 **전에**, 같은 사실이 본 마일스톤 원장에 이미 있는지 본다. 동일성 기준은 «`<라벨> <file:line> <증상>`»(§1.0 3-1 과 같다)이며 preflight 정규식 발화처럼 file:line 이 여럿인 항목은 **경로 집합**으로 비교한다. **이미 있으면 `status` 가 `open` 이든 `resolved` 든 새 ID 를 만들지 않는다** — 대신 그 항목에 `- 재발화: <YYYY-MM-DD> (stabilize <N>회차) — 판정 유지` 한 줄만 붙인다. 경로 집합이 달라졌으면 새 항목이다. **이 규칙이 없으면 preflight 의 구조적 오탐이 매 라운드 새 ID 로 재생산된다**(dogfood Round 11 실측 — raw-hex 5건·voice 10건이 3회차 내내 동일하게 재발화).
...
- **검증 레벨 분포는 산하 task의 `## 6-1`에서 직접 집계한다** — 채점표에도 `- closure`에도 그 값이 없으므로(closure는 `기계AC`·`자동화율`만 담는다) 커밋된 `## 6-1`이 유일한 항구 출처다. 판정 규칙은 `/validate-workitem`과 같다(**`[자동 테스트]` AC만 레벨 대상**, 명시 `{레벨}` 태그 우선, 미기재는 테스트 경로 추론, 나머지는 `레벨 비대상`).
...
- AC 검증 레벨 분포: unit N / integration M / contract K / e2e L · 레벨 비대상 P  (`[자동 테스트]` AC만 레벨 대상 — 출처: task `## 6-1`. `[산출물 검사]`·관측 2종은 «레벨 비대상»이며 자동화율 분류와는 별개다 — ADR-065 D4)
...
- AC 검증 레벨 분포: unit 20 / integration 9 / contract 2 / e2e 1 · 레벨 비대상 4
...
fan-out 병렬 오케스트레이션 정책은 [ADR-075](../../../docs/90-decisions/boilerplate/ADR-075-main-session-orchestration-v2.md) D2 정합.
