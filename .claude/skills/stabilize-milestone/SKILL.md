...
3. **FAC ↔ AC unmapped 검출** ([ADR-037](../../../docs/90-decisions/boilerplate/ADR-037-spec-coverage-audit.md)#amend-1 영속 SSOT `## 7-1` 정합):
   - 본 마일스톤의 모든 feature 문서 `## 7-1. FAC ↔ AC 매핑표`에서 *unmapped* 또는 *비어 있음* 항목 회수. **우변이 `승인 스냅샷 <경로> (manifest: …)` 형식인 행은 unmapped 가 아니다**(ADR-037#amend-4 — 승인 표면이 검증자다). 우변이 비었거나 `(해당 task 없음)` 처럼 **검증자를 지목하지 않은** 행만 센다.
