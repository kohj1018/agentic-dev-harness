...
- Flutter: `lib/theme/tokens.dart`·`lib/theme/app_theme.dart`(ThemeData/ColorScheme/TextTheme 배선) + `lib/prototype/theme_gallery.dart` 진입 파일(같은 섹션 순서) + `test/prototype/theme_gallery_test.dart`(프로필 뷰포트 렌더 + Accessibility Guideline 4종 + overflow 0 + 스냅샷 PNG). **폰트·아이콘 적재 (ADR-073#amend-1)**: 위젯 테스트는 pubspec 선언 폰트를 자동 적재하지 않는다 — `test/support/load_fonts.dart` 를 두고 `setUpAll(loadAppFonts)` 로 부른다. **`FontManifest.json` 을 읽어 선언된 패밀리를 전부 적재**한다(패밀리를 손으로 나열하면 폰트를 추가할 때마다 뒤처진다). **`MaterialIcons` 도 그 매니페스트에 있다** — 빠뜨리면 체크 표시·「+」가 네모로 남는다. 적재하지 않으면 스냅샷이 육안 대조에 못 쓰일 뿐 아니라 **`meetsGuideline(textContrastGuideline)` 의 판정이 뒤집힌다**(tofu 는 획이 굵어 통과하고 실제 글꼴은 실패한다 — Round 12 실측).
- **폰트는 «전달 방식»까지 배선한다 (ADR-073#amend-1 결정 1·2 — 필수)**. 폰트 패키지·파일(예: `@fontsource/*`·`pretendard`, `assets/fonts/`, Flutter `pubspec.yaml` `fonts:`)은 여기서 추가한다(설치 소유 예외 — ADR-071 D6).
  ...
  - **패키지 CSS 를 `@import` 하는 것으로 끝내지 마라** — `@import "pretendard/dist/.../pretendardvariable.css"` 는 그 파일의 상대 `url()` 을 번들러가 자산으로 잡지 않아 **선언은 있는데 빌드에 폰트 파일이 없는** 상태가 된다(Round 12 실측 — 게이트 정적 검사는 통과하고 런타임 검사만 잡았다). **폰트 파일을 프로젝트 소스 안으로 복사하고 자기 CSS 에 `@font-face` 를 써라.** 라이선스 파일도 같이 둔다.
