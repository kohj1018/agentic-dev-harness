...
0. **스캐폴드 (green-field scope 한정 — ADR-071 D5)**: scope(registry `cat-common-repo-layout`)마다 «등록 소스 루트 0 **그리고** 프레임워크 manifest 부재»면 아래를 수행하고, 아니면 `## Scaffold`에 `skipped (<사유>)`를 적고 다음 scope로. 전 scope 처리 후 수행 1로 간다.
   ...
   2-0. **scope 하위 harness 파일 탐지 (monorepo 필수 — dogfood Round 12)**: 2-1 의 보호 경로 목록은 **저장소 루트 기준**이라 `apps/web/AGENTS.md` 같은 것을 보지 못한다. 복사 직후 그 scope 트리에서 harness **이름**(`AGENTS.md` `CLAUDE.md` `.claude/` `.codex/` `.agents/` `.boilerplate/`)을 찾아 발견하면 `Scaffold harness-file in scope: <경로> — <추정 원인>` 을 출력하고 `STACK_SETUP_PLAN ## Scaffold` 의 그 행 «제외·충돌» 칸에 적는다. **자동 삭제하지 않는다** — 생성기가 매 실행 되살리는 경우가 있고(Next.js 의 `next dev`) 되돌리기는 사용자 결정이다. 왜 보고해야 하는가: 저장소 안에 **두 번째 `AGENTS.md`** 가 생기면 그 디렉터리를 cwd 로 일하는 에이전트가 보일러플레이트 규율 대신 그것을 읽는다(실측: `create-next-app` 이 `apps/web/AGENTS.md` + `CLAUDE.md` 를 생성, 루트 201파일 해시는 완전 동일).
...
2-1. **harness 경로 배제 (ADR-063 D2)**: 생성하는 도구 config 중 **formatter / linter / 타입 검사 include / 테스트 커버리지 집계 / 의존성 그래프**의 검사 범위에서 아래를 제외한다 — 이들은 프로젝트 소스가 아니라 agent harness다.
   ...
   - **design gate 산출물** `design-gate-shots/`·`design-gate-storybook/` — monorepo 면 **그 scope 안에** 생기므로 scope 도구 config 에도 넣는다(6-4-1 참조).
...
3. `docs/00-meta/STACK_SETUP_PLAN.md`을 다음 규칙으로 처리한다:
   ...
   - 본 skill이 채울 섹션:
     ...
     - `## Dependency Tools` — **보완만**(ADR-075 D14 수행-6-2-0): 표·행이 없으면 관측 신호로 채우고, `/bootstrap-stack`이 기록한 행은 덮어쓰지 않는다(불일치는 출력 보고 + 사용자 결정).
...
6. **Toolchain 선설치 + E2E readiness** (실행 순서상 step 5 smoke test *앞*에 수행 — `allowed-tools` 의 Bash 활용, 신규 권한 불필요):
   ...
   - **6-2-1. 테스트 격리 권장 (ADR-075 D9)**: 생성하는 e2e/통합 설정에 *가능한 범위에서* 격리를 권장한다 — playwright `webServer`는 동적 포트, 통합 테스트는 트랜잭션 롤백/임시 스키마/testcontainers. stack-guard가 unit-test 격리를 직접 authoring하긴 어려우므로, 미보장 시 `STACK_SETUP_PLAN.md`에 "테스트 격리 미설정 — 병렬 builder 시 foreman 순차 권장" 1줄 부기(implement partition이 실제 보호).
   - **6-2-b. 기초 라이브러리 baseline 설치 (ADR-071 D6)**: registry에서 `disposition: 확정` **그리고** 카탈로그 `설치: baseline`인 행의 패키지를 **scope별로**(그 scope의 행을 그 scope의 PM으로 — ADR-071 D2 `(scope, id)` 키) 설치한다(UI 킷·스타일링·아이콘·UI 미리보기 도구·계측 SDK 등 — 폰트 제외). 버전은 registry `확인일`의 researcher 고정값. `설치: task` 행은 설치하지 않는다(plan-workitem → implement). 설치 실패는 6-5 `Needs Install` fallback.
     ...
     - **생성형 UI 킷(`cat-web-ui-kit` — shadcn/ui 등)**: 킷 자체는 설치한다(컴포넌트 코드가 import 한다). 다만 그 `init` 이 스타일시트에 쓴 **토큰·팔레트·폰트 변수 블록은 손대지 않고 기록만 한다** — `STACK_SETUP_PLAN` 에 `킷 토큰 블록 — R6-1 재배선 대상: <파일>` 한 줄. 그 시점에는 DESIGN `## 2`·`## 3` 이 없어 그 값들은 결정이 아니라 자리표시자이며, `/bootstrap-design` R6-1 이 킷 변수를 DESIGN semantic 토큰의 **별칭**으로 재정의해 출처를 하나로 만든다(ADR-071 D6). **킷이 끌고 오는 번들 의존(`class-variance-authority` 등)은 결정의 일부이므로 제거하지 않는다** — ADR-071#amend-1 의 「결정 밖 애드온 제거」는 *선택 가능한 애드온*(Storybook 의 Chromatic·vitest·docs·mcp)을 가리킨다. registry 그 행에 번들 의존 목록을 적는다.
   ...
   - **6-4. `validate:e2e` scaffold (runtime target 기준, e2e 필요 시)**:
     ...
     - **target 에 `native/*` 포함**: 아래 셋을 모두 한다. **Playwright 에 배선하지 않는다** — `package.json` 이 design gate 때문에 존재하더라도 그것을 앱 e2e 의 근거로 삼지 않는다(ADR-059 D8).
       ...
       3. `validate:e2e` 진입점을 **`flutter test integration_test` 로 박는다 — `-d` 를 넣지 않는다.** 이유: `-d` 는 device id 또는 이름 접두사만 받으므로 `emulator-5554` 처럼 재부팅하면 달라지는 값이나 `연결된 Android device 1대` 같은 자연어를 넣으면 명령 자체가 성립하지 않는다. `-d` 를 생략하면 러너가 **`integration_test` 지원 device 로 후보를 좁힌 뒤** 하나만 남으면 자동 선택한다(실측: android emulator + windows + chrome + edge 4개가 붙은 상태에서 emulator 자동 선택, 오류 없음). 후보가 둘 이상 남을 때만 호출 측이 `npm run validate:e2e -- -d <id>` 로 지정한다. 어떤 device 를 골라야 하는지의 *규칙*은 registry 에 적는다. (참조 갱신: target이 둘 이상이면 target별 진입점은 `scripts/e2e-target.mjs`가 실행 시점에 id를 해석한다 — ADR-059#amend-1 결정 1. 진입점 문자열에 id를 박지 않는 원칙은 불변)
       4. **`--machine` 을 진입점에 박지 않는다** — 사람이 읽을 때는 기본 리포터가 낫고, 판정하는 쪽이 `npm run validate:e2e -- --machine` 으로 덧붙여 다시 호출한다(ADR-059 D4).
       5. **target이 둘 이상이면** `scripts/e2e-target.mjs`를 생성한다(ADR-059#amend-1 결정 1) — 입력: `node scripts/e2e-target.mjs <android|ios> [-- <flutter test 인자…>]`. 동작: registry `## E2E Smoke Registry`의 그 target 행 실행 대상 선택 규칙을 `flutter devices --machine` 출력에 대입해 device id를 얻고, **`--` 뒤 인자를 그대로 써서**(없으면 기본 `integration_test`) `flutter test <그 인자들> -d <id>`를 실행하며 `--machine` 스트림을 그대로 전달한다. **resolver는 device 선택만 더한다 — 테스트 경로·`--flavor` 같은 옵션을 삼키지 않는다.** 후보 0개면 `BLOCKED_ENV` 종료코드·메시지로 끝낸다. `validate:e2e:android`/`validate:e2e:ios` 진입점은 이 스크립트를 부르고, `validate:e2e`는 선언된 target을 순차 호출하는 집계 진입점으로 둔다.
     - **공통**: *같은 계열로 이미 존재* 하면 덮어쓰지 않고 발견 사실만 출력에 기록한다(도구 감지 우선순위 정합 — 기존 도구 미덮어씀). **target이 둘 이상이면** `validate:e2e:<target>`을 target마다 만들고(native는 `node scripts/e2e-target.mjs <target>` — device id를 실행 시점에 해석하며 `-d` 리터럴은 어디에도 박지 않는다) `validate:e2e`는 집계 진입점으로 배선한다(ADR-059#amend-1). 단일 target은 기존과 같다. 기존 `validate:e2e`가 한 계열로만 물려 있고 새 target이 추가되면 그 계열을 `validate:e2e:<target>`으로 옮기고 집계를 새로 만든다(덮어쓰기 아님 — 이동 사실을 출력에 기록). **native를 옮길 때는 기존 명령의 테스트 경로·옵션(`--flavor` 등)을 `node scripts/e2e-target.mjs <target> -- <그 인자들>`의 `--` 뒤로 그대로 옮긴다** — 고정 명령으로 바꾸면 옵션이 사라지고, 명령만 통째로 옮기면 device 해석이 빠진다. 이 e2e provision/smoke 는 milestone graduation 의 E2E hard-block(ADR-052#amend-1 / ADR-068 D3 item 3)이 검사할 대상을 선readiness 한다.
   ...
   - **6-4-1. design gate v3 어댑터 + Visual-QA scaffold (UI 한정 — ADR-072 D6 / ADR-058#amend-3)**:
     ...
     - **Storybook 정적 빌드 출력**: `design-gate-storybook/`을 `.gitignore`에 추가(첫 실행 전). **monorepo 면 그 scope 에도 추가한다** — 게이트는 각 화면의 `scope` 를 작업 디렉터리로 쓰므로 빌드 출력이 `apps/web/design-gate-storybook/` 처럼 **scope 안에** 생긴다. **루트 `.gitignore` 만으로는 부족하다**: git 은 그 패턴을 어느 깊이에서든 무시하지만, scope 의 도구(예: `useIgnoreFile` 을 켠 Biome)는 **자기 루트의 ignore 파일만 읽어** 빌드 산출물을 format·lint 대상으로 삼는다(dogfood Round 12 실측 — `validate` format 단계가 빌드 파일을 훑다 사실상 멈췄다). 그래서 scope 의 `.gitignore` **와** 그 scope 도구 config 의 제외 목록 **양쪽에** `design-gate-storybook/`·`design-gate-shots/` 를 넣는다(수행 2-1 harness 경로 배제와 같은 자리).
     ...
     - **자가 검사(최대 5케이스 — 웹 전용 프로젝트는 4)**: `<pm> validate:design -- --self-test` 실행(Flutter scope가 루트가 아니면 `--scopes apps/mobile`처럼 fixture를 만든 scope를 함께 넘긴다 — 루트 `pubspec.yaml`만 보면 monorepo에서 (d)가 조용히 빠진다) → 위 판정 행. **(d) Flutter fixture 케이스는 `pubspec.yaml` 이 있는 scope 에서만 돈다 — Dart 가 없는 웹 전용 프로젝트의 정상 결과는 (a)(b)(c)(e) 4케이스 PASS 이며 그것이 «자가 검사 통과»다.** **(e) 렌더 조건 케이스**(ADR-072#amend-5 결정 3)는 스택과 무관하게 항상 돈다. 케이스 수로 통과를 판정하지 않고 *실행된 케이스가 전부 기대와 같은가*로 판정한다(ADR-072#amend-1). **capability 버전 핸드셰이크·고정 적합성 oracle은 없다**(ADR-072 D6이 ADR-058#amend-2를 대체). 복사 직후 canonical asset의 SHA-256을 registry `copied-from`에 적는다(caller 대조 없음).
...
| `## Design Gate Adapter` | 매 실행 **자가 검사 재실행**. canonical asset sha ≠ registry `copied-from`이면 갱신 대상 — project 사본 sha == `copied-from`(무수정)일 때만 새 canonical로 교체 + `copied-from` 갱신, 사본 sha == canonical sha면 코드 동일 — copied-from만 갱신(ADR-063#amend-1 결정 3); 다르면(local modification) 덮어쓰지 않고 diff 보고 + 사용자 결정 |
