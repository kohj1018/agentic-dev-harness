...
1. **누구에게 말하나** — `docs/10-charter/DISCOVERY.md` `## 2. 페르소나` + `## 5. JTBD` + `## 6. 시나리오`. **DISCOVERY 는 선택 산출물이라 부재할 수 있다**(`/discover-product` 미실행) → 그때는 `PROJECT_CHARTER.md` `## 2.1 페르소나` / `## 3.1 핵심 시나리오` 로 대체하고 그 사실을 출력에 1줄 명시한다. **둘 다 비어 있으면 판단하지 않는다.**
2. **무엇을 해결한다고 말하나** — `docs/10-charter/PROJECT_CHARTER.md` `## 3. 해결하려는 문제`.
