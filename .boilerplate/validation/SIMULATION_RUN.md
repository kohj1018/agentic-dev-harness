...
본 라운드의 harness 변경(ADR-070#amend-1 · ADR-004#amend-6 · ADR-071 D6 확장 · ADR-072 D5-3-1·#amend-3 · skill 8종 · agent 12종) **뒤에** 부록 E 스크립트를 다시 돌렸다. (현재 SSOT: ADR-074)
...
- **R4 코드 초안**: builder 2 dispatch 가 **둘 다 45턴 상한에서 보고 0건**으로 멈췄다(발견 59 → ADR-004#amend-7). 작업은 거의 끝나 있었고 죽은 자리는 최종 검증이다. (현재 SSOT: ADR-074)
...
- **ADR-004 는 임계를 넘었다 — 9 개정.** ADR-045 D6 은 「개정 8개 이상 누적 → 통합 재발행(supersede)」이고 D6 의 grandfather 조항은 *ADR-045 이전에 이미 누적된* ADR 에만 적용된다. ADR-004 의 amend-4~9 는 **전부 Phase 7 이후**이므로 grandfather 대상이 아니다. **다음 변경은 amend 가 아니라 통합 재발행이어야 한다.** 본 라운드에서 재발행하지 않은 이유는 amend-8·9 가 같은 날 falsifier 발화로 연쇄해 **정책이 아직 안정되지 않았기 때문**이다 — 재발행은 amend-9 falsifier 가 판정된 뒤(Round 13)가 맞다. 그 사실을 여기 남긴다. (현재 SSOT: ADR-074)
- **개정 수와 falsifier 발화가 같이 간다.** 5개정 ADR-004 가 4회, 4개정 ADR-072 가 1회 발화했고 1개정 ADR 들은 0~1회다. 발화가 나쁜 신호가 아니라 **사전 등록이 실제로 작동한 신호**다 — 004 의 4회는 전부 규정된 응답을 냈다. (현재 SSOT: ADR-074)
...
| ADR-004#amend-7 | (a) `maxTurns: 60` 에서도 builder 보고 0건 상한 도달 1회 / (b) 중간 보고가 자의적 절반으로 매번 초반 | **(a) builder 미측정 · (b) 반대 방향으로 실패** | builder 는 amend-7 적용 후 dispatch 가 아직 없다. (b)는 「자의적 절반」이 아니라 **중간 보고가 0건**이다 — 관측 4/4(builder 2 + planner 2)에서 한 번도 나오지 않았다. planner 2건은 amend-7 문구를 본문·dispatch 양쪽에 받은 상태였다(발견 64 → amend-8) (현재 SSOT: ADR-074) |
| ADR-004#amend-8 | (a) 결정 1~4 뒤에도 쓰기 에이전트가 보고 0건으로 상한 도달 1회 | **미측정 (규칙 신설 당일)** | Round 13 대상. 이 falsifier 가 ADR-047 D3 «예산 영향» 항목 채택의 **조건**이다 (현재 SSOT: ADR-074) |
...
| ADR-004#amend-5 | `maxTurns: 45` 에서도 상한 중단이 2회 이상 반복 | **미측정** | Round 12 builder 재측정 미도달 (현재 SSOT: ADR-074) |
...
- **팬아웃 단위가 상한을 소진한 3건이 전부 ADR-004#amend-6 적용 *전*이었다.** amend-6 적용 후 dispatch 한 designer(브리프 3종)·reviewer(브리프 비평)는 둘 다 보고를 완료했다 — 다만 n=2 라 아직 신호다. (현재 SSOT: ADR-074)
...
| 개정된 ADR | 8종 / 신규 amendment **14건** (ADR-004 5 · ADR-072 4 · ADR-037 1 · ADR-047 1 · ADR-070 1 · ADR-071 1 · ADR-073 1) (현재 SSOT: ADR-074) |
| falsifier 판정 | 30행 — 발화 **6건**(ADR-004 4 · ADR-072#amend-2 부분 1 · ADR-073#amend-1 1) / 미발화 15 / 미측정 9 (현재 SSOT: ADR-074) |
...
### ① ADR-004 통합 재발행 (supersede) (현재 SSOT: ADR-074)
...
회수 문서 15개(>10)에도 reviewer subagent dispatch 없이 in-session으로 완주했다. 판정 결과 자체(P0 2·P1 9·P2 3, 발견 14건)의 신뢰도 저하는 관측되지 않았으나, ADR-004#amend-9 결정 4 (현재 SSOT: ADR-075 D11 (b))에 명시된 임계와 실행이 어긋난다. 「회수 방식이 grep·부분 읽기 위주면 단일 세션도 허용」으로 완화할지, dispatch를 실제로 강제할지 — 그 정책 판단은 이번 라운드 범위 밖이라 결정하지 않고 사실만 기록한다.
...
### builder 재측정 (첫 과제 ②) — 조건당 새 세션, T-002-today-list-wiring 동일 slice

| 조건 | maxTurns | effort | 소요 | tool_uses | 반환 시점 컨텍스트¹ | 실제 출력 토큰² | 완료 AC | validate | 회수 턴 | Red 보고 | 첫 쓰기 이전 입력 수 |
|---|---:|---|---:|---:|---:|---:|---|---|---:|---|---|
| (a) 현재 기준값 | 60 | — (상속) | **비교 불가**³ | 36 | 84,767 | 26,044 | 3/3 | OK | 0 | 완전 — AC 3건 전부 실제 어설션 실패 인용 | **15** (Bash 14 · Read 1 — 1건은 `flutter --version` 환경 확인) |
| (b) effort만 | 60 | medium | 280,985ms | 37 | 73,896 | 18,464 | 3/3 | OK | 0 | 완전 — 자기 작성 테스트의 기대값 버그(`elapsed_ms_since_launch`)까지 스스로 발견·수정 | **11** (Bash만 — Read/Glob 0) |
| (c) 턴만 낮춤 | 20 | — (상속) | 367,498ms(189,480+178,018) | 33(20+13) | 78,517 | 25,387 | 3/3 **(단, 아래 ⑤ — AC-2가 실앱에서 공허하다)** | OK(무관 pre-existing 실패 1건 별개 — `design_gate/self_bad_test.dart`) | **1** | **무효** — 1차 상한 도달 시 구현 3개 파일이 이미 완성돼 있었고 재개 후 테스트를 뒤늦게 작성해 진짜 Red 미관측. 다만 builder가 이 사실을 스스로 정직하게 보고(가짜 Red 주장 없음) | **11** (Bash 10 · Read 1 — 전부 입력 열람) |
| (d) 둘 다 | 20 | medium | 371,531ms(297,820+73,711) | 29(20+9) | 62,242 | 15,477 | 3/3 | OK(17/17) | **1** | 완전 — **1차 구간에서** 테스트 작성(#12) → `flutter test` Red 관측(#13) → 수정 반복 후 상한 도달. AC 3건 전부 실제 어설션 실패 인용 | **8** (전부 Bash 입력 열람 — 9번째 호출의 `cat >` heredoc이 첫 쓰기 시도) |

¹ task-notification이 「subagent 토큰」으로 보고하는 값이다. 실행 로그와 대조한 결과 **마지막 요청 1건의 `cache_read + cache_creation + input + output` 규모**이며 **누적 사용량이 아니다**(예: (a)의 마지막 요청은 `cache_read 83,799 · cache_create 156 · in 2 · out 1,535`). 즉 이 열은 «끝날 때 컨텍스트가 얼마나 커져 있었나»를 뜻한다 — 비용·총사용량 대리값으로 읽으면 안 된다. 앞선 라운드 기록의 「누적치」 해석은 오류이며 여기서 정정한다.

² 요청 ID로 중복 제거한 **누적 `output_tokens`**다. 네 조건의 누적값 전체는 — (a) 요청 32 · out 26,044 · cache_create 139,692 · cache_read 1,840,821 / (b) 35 · 18,464 · 58,454 · 1,888,388 / (c) 34 · 25,387 · 63,265 · 1,870,673 / (d) 30 · 15,477 · 83,887 · 1,279,044. `cache_read`는 세 조건이 1.84~1.89M로 사실상 같아 변별력이 없고, (a)의 `cache_create` 139,692는 아래 ³의 재접속으로 캐시가 다시 만들어진 몫이 섞여 있다. 따라서 **조건 간 변별이 되는 축은 `output_tokens` 하나뿐**이다.

³ (a)는 dispatch 도중 사용자 측 네트워크 단절로 세션이 끊겼다 재접속됐다(turn 상한 도달이 아니다). 실행 로그(`agent-a82aeadd58ada77e7.jsonl`)의 전체 wall은 6,029초(08:21:13→10:01:42)이고 그 안에 30초 초과 공백이 6개, 가장 큰 둘이 25.6분(08:31:15→08:56:54)과 63.4분(08:56:54→10:00:16)이다. **단절이 몇 번이었는지는 로그로 확정할 수 없다** — 두 공백 사이의 `08:56:54` 기록은 백그라운드 `find /` 명령이 **메모리 부족으로 종료됐다는 task-notification**이지 연결 복구의 증거가 아니다. 활성 시간 재구성은 기준에 따라 **233초(30초 이하 간격만 합산) ~ 688초(5분 초과 공백만 제외)** 사이로 흔들리므로 **(a)의 소요는 다른 조건과 비교할 수 없다**. tool_uses(24+12=36)는 로그의 tool_use 이벤트를 직접 세어 task-notification 두 값의 합과 일치 확인했다.

### 프로토콜 이탈 — (c)·(d) 프롬프트가 바이트 동일이 아니었다

네 프롬프트를 로그에서 꺼내 루트 경로만 정규화하고 대조했다. (a)·(b)는 **바이트 동일**, (c)·(d)에는 `## 보고 형식` 끝에 **5번 항목이 하나씩 더 붙어 있었고 둘의 문구도 서로 달랐다** — (c) 「결과가 오면(완주든 상한 중단이든) 그대로 전부 나에게 붙여넣어줘 — 상한에 걸렸다면 재개할지 여부는 내가 판단해서 다음 지시를 줄게.」 / (d) 「… 마찬가지로 제가 디스크 상태 먼저 확인하고 재개 지시를 줄게.」. 문맥상 **사용자가 foreman(메인 세션)에게 준 지시가 builder dispatch 프롬프트에 섞여 들어간 것**이다.

**영향 평가**: 이 줄이 건드릴 수 있는 유일한 측정 대상은 상한 도달 시의 보고 행동이다. 그런데 아래 절에서 그 보고가 **애초에 성립 불가능**함이 확인됐으므로, 프롬프트가 깨끗했더라도 결과는 같았다 — **(c)·(d) 재측정은 불필요**하다고 판단한다. maxTurns 절단(기계적)·tool_uses·토큰·완료 AC는 이 줄의 영향을 받지 않는다. 다만 **다음 라운드의 실험 절차는 프롬프트를 파일에서 그대로 읽어 보내고, dispatch 직후 로그에서 바이트 동일을 확인하는 단계를 넣어야 한다** — 「바이트 동일하게 쓴다」는 지시만으로는 지켜지지 않았다.

### 판정 (모델·예산 정책 재발행본이 승계할 `effort` 3갈래 규칙 적용)

- **maxTurns**: (c)·(d) 둘 다 `tool_use` **정확히 20**에서 잘렸다(각각 총 33·29가 필요했다) → **「maxTurns 60 유지」 확정**(amend-5 falsifier 판정 재확인).
- **effort**: (a)의 소요가 오염돼 비교 불가이므로 **누적 `output_tokens` 하나로만 판정한다**. (b)는 (a)와 작업량이 사실상 같으면서(tool_uses 37 대 36) 출력 토큰이 **29.1% 적고**(18,464 대 26,044), (d)는 상한 도달·재개를 거치고도 **40.6% 적다**(15,477). 완료 AC·validate는 넷 다 통과다. Round 11(웹, medium이 ~2배 느리고 비쌈)과 **방향이 반대**다 — 값을 뒤집지 않고 **모델·예산 정책 재발행본의 `effort` 절**에 **「스택별 상이 — 결론 보류, Round 14 재측정」**으로 적는다. **(a)가 오염 표본이고 축이 하나뿐이라는 점이 보류의 추가 사유다** — 다음 측정은 (a)부터 다시 잰다.
- **n=2의 의미**: Round 11(웹·Next.js)과 Round 12·13(Flutter)은 플랫폼·task가 달라 「같은 결론이 다른 스택에서도 서는가」로만 읽는다. 셀별 엄밀한 n=2가 아니다.
- **완료 AC 「3/3 동일」의 한계(⑤)**: (c)는 컨트롤러가 `onEvent` **선택 콜백**으로만 이벤트를 넘기는데 `main.dart`가 그 콜백을 주입하지 않아 **실앱에서는 `habit_checked`가 아무 데도 기록되지 않는다** — 테스트만 콜백을 주입해 통과한다. (a)는 `HabitCheckedEvent` 값 객체, (b)는 `lastHabitCheckedEvent` 필드, (d)는 `events` 리스트로 **컨트롤러 안에 남겨** 주입 없이도 관측된다. 즉 **(c)의 3/3만 실앱 계측이 공허하다**. effort 비교((b)·(d) 대 (a))와 maxTurns 판정에는 (c)의 이 결함이 들어가지 않으므로 두 판정은 유지되지만, 「네 조건 완료율 동일」로 뭉뚱그리지 않는다. (제품층 결함이라 복제본 수리 대상은 아니다 — 실험 해석의 한계로만 적는다.)

### falsifier 판정 — 부분 보고 형식은 «지켜지지 않은» 것이 아니라 «성립 불가»다

**관측(실행 로그 원문 대조)**: (c)·(d) 모두 1차 구간이 `tool_use` 20에서 잘렸고, 그 시점의 반환문이 부분 보고 형식(「쓴 파일 목록 + 남은 것 1줄」)이 아니라 **작업 도중의 사고 조각**이었다 — (c) 「Looks plain. Let me use sed directly with line number to be safe.」, (d) 「Good, `AppTheme.light` exists. Now update manifest.json product_entry.」.

**원인 규명 — 이것이 이번 라운드의 핵심 발견이다.** 두 조건 모두 **마지막 assistant 텍스트가 절단 지점보다 2호출 앞**이다. 즉 에이전트는 마무리 턴을 받지 못하고 **작업 중간에 그대로 잘렸다**. 그리고 네 조건의 로그 전체에 **상한 접근을 알리는 메시지가 0건**이다(`maxTurns` 문자열은 system prompt 스냅샷 안에만 있다). D9-1이 스스로 「턴 수를 세려 하지 마라 — 너에게 『지금 몇 턴째인가』를 보여 주는 것은 없다」고 못박은 것과 합치면 결론은 하나다 — **「상한에 닿았음을 알 수 없는 에이전트」에게 「상한에 닿으면 이 형식으로 보고하라」고 요구하는 것은 자기모순이며, 그 지시는 어떤 에이전트도 만족시킬 수 없다.**

| 사전 등록 falsifier | 판정 | 근거 |
|---|---|---|
| 재발행본 초안 Mutation Contract (a) 중 「상한에 닿은 dispatch가 D9-3 형식을 내지 않음」 | **발화 (2회)** | (c)·(d) 위 관측 |
| 같은 (a) 중 「상한 중단 + 산출물 파일 0건」 | **미발화** | (c)는 상한 시점에 구현 3파일, (d)는 컨트롤러·테스트가 이미 디스크에 있었다 |
| ADR-004#amend-8 (a) 「쓰기 도구 보유 에이전트의 보고 0건 상한 도달」 | **발화 (2회)** | 반환은 있었으나 보고가 아니다 (현재 SSOT: ADR-074) |
| ADR-004#amend-9 (a) 「상한 중단 + 산출물 파일 0건」 | **미발화** | 위와 같음 (현재 SSOT: ADR-074) |
| ADR-004#amend-9 (b) 「골격만 쓰고 미검토 절반 초과」 | **미발화** | `미검토:` 0건 (현재 SSOT: ADR-074) |

**사전 등록된 대응**(「D9를 되돌리고 slice 강제만 남긴다」 / 「예산·지시를 더 만지지 말고 slice 강제만 남기고 dispatch를 기계적으로 쪼갠다」)**을 그대로 집행한다** — 사용자 확정(2026-09-13). 재발행본의 D9에서 (1) 중간 보고 지시 (2) write-first (3) 부분 보고 형식 세 문장을 에이전트 본문에서 삭제하고, 「턴 수를 세려 하지 마라」(amend-6 재발 방지용 부정 지시)와 builder의 「slice 산출물 4개 초과면 착수 전 보고」(미측정)만 남긴다. **부분 보고의 자리는 호출자 규칙으로 이관한다** — 상한 중단 시 foreman이 워킹트리를 직접 읽어 「쓴 파일 목록」을 만든다. 이번 (c)에서 실제로 그렇게 복구했고 작동했다.

### write-first(D9-2) — 표본 n=5, 5/5 미준수
...
| ADR-063#amend-1 | (b) (ii)가 실제로는 canonical 회귀였던 사례 | **미발화 — (iii) 경로는 합성 주입으로 검증** | P8-3: dogfood-flutter `copied-from` 끝 1글자를 **의도적으로** 바꾼 뒤 재실행 → (iii) «코드 동일·기록값만 낡음»으로 판정돼 `copied-from`만 갱신(합성 테스트 통과 — 실사례 포착 아님). 자연 발생 (iii)는 amend 전인 2026-09-12 Phase 0에서 1회 관측됐다(위 Round 13 첫머리 기록) |
...
| ADR-050#amend-2 | Needs Script가 마일스톤당 3회 이상 | **미발화(관측 범위 내)** | P8-3 (c)/(e): 실행 보고에는 없었으나 사후 로그로 5 dispatch 전부 scratch 경로 포함을 확인했고, 실행 뒤 dogfood-web 워킹트리에 qa·reviewer가 만든 신규 파일 0건(변경은 stabilize·repair-plan이 쓴 원장 2종 + `next dev`의 `AGENTS.md` 변조뿐). `Needs Script` 보고 0회 — 새로 연 항목 5(보고 누락)는 유지 |
| ADR-035#amend-4 | Charter 공급 절 변경을 판정이 놓침 | **미확인** | 판정 명령(`git log -1 … PROJECT_CHARTER.md` → `git diff … DISCOVERY.md` hunk 대응)이 실행된 것은 사후 로그로 확인했으나 발화·침묵 결과가 보고에 없고, Charter 공급 절 변경 자체가 없는 저장소라 놓침 여부를 판정할 입력이 없다 — 새로 연 항목 5 |
...
| falsifier 판정(Round 13 전체) | 발화 4건(ADR-004#amend-8, 그리고 ADR-004#amend-9 결정 1의 «미준수» 자체는 falsifier가 아니라 규칙 폐기 근거) / 미발화 9건(그중 2건은 «정상 동작이 실사례로 검증됨», ADR-063#amend-1은 합성 주입 검증, ADR-050#amend-2는 사후 로그 확인으로 미확인에서 전환) / 미확인 1건(ADR-035#amend-4 — 결과 미기재) / 미측정 6건 (현재 SSOT: ADR-074) |
...
ADR-004#amend-5 결정 4 가 규정한 n=2 재측정을 시작했으나 **조건 분리가 성립하지 않아 중단했다.** `.claude/agents/builder.md` 를 조건별로 편집해도 **그 세션의 dispatch 에 반영되지 않는다**(0-tool probe 로 확정 — 발견 74). 조건 (b)(c)(d)는 이 세션에서 측정 불가다. (현재 SSOT: ADR-074)
...
### ADR-004#amend-4 결정 4 적용 — **규칙에 공백이 있다** (현재 SSOT: ADR-074)
...
| **035** DISCOVERY | 1 / 4 | 미확인(결과 미기재 — 새로 연 항목 5) | 부분 — 규칙 반영 + 판정 명령 실행은 로그로 확인, 침묵 결과는 미기재 | 4 |
| **037** spec coverage | 0(참조 갱신만) / 4 | — | — | 4 |
| **050** lifecycle skills | 1 / 2 | 미발화(관측 범위 내) | ✅ 사후 로그 확인 — 5 dispatch 전부 scratch 경로 포함 + 신규 파일 0 | 2 |
...
| **059** Flutter 프로파일 | 1 / 2 | 미발화(생성 후 1회 실행 통과 — CI 러너 미트리거) | ✅ P8-2 보완 실행에서 `validate:ci` 생성·1회 실행(`@Tags` 테스트 0건이라 제외는 아직 no-op) | 2 |
| **063** verification harness | 1 / 1 | 미발화(합성 주입 검증) | ✅ P8-3 합성 주입으로 (iii) 경로 실행 | 1 |
