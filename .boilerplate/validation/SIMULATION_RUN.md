...
- **이제 실측된 스테이지**: discover→bootstrap→stack→guard→design→plan-milestone(R5)→plan-workitem(배치)→implement×4→validate→finalize→stabilize(§3-V)→repair-milestone→repair-workitem(sticky + P2 배치 corrupt-clear/hover/rise/a11y/테스트)→**validate-workitem fan-out**→re-stabilize→졸업. 관통 커버리지 완결.
- **validate-workitem fan-out 실측 (INST-6 메커니즘 최초)**: 외부 리뷰 라운드가 "repair 후 validate-workitem 재실행 누락 + repair diff는 fan-out 대상"을 지적 → 3축 fan-out(validator×2 axis1-2·5 + qa axis8) 실행 → **inline foreman(나)이 놓친 2 P1을 catch**: (1)[Doc-code-mismatch] corrupt-clear(`setCorrupt(false)`)가 T-004 §8 "corrupt 경고 세션 유지" 결정을 문서개정 없이 역전, (2)[Repair-bookkeeping-gap] P2 배치 status 미토글·§5 로그 누락. + seam INV-1/2/3 안전 확인·대비 3.70:1 독립 재계산(AA 4.5:1 미달 residual). M1-repair-2로 해소(§8 개정·status 토글·DESIGN §7 doc-drift). **fan-out 메커니즘 가치 + "커밋 전 다관점 검증" 규율을 이중 실증** — 초기 NO→YES(§3-V)에 이은 두 번째 게이트 실효 사례. 단 *per-task large diff*(T-002~004)는 여전히 inline이었으므로 large-diff fan-out 자체는 소급 미실행.
...
- **graduation 불변**: fan-out이 잡은 2 P1은 문서/북키핑 정합(코드 결함·[Experience-drift]·P0 아님)이라 graduation 기준 불변 — M1 **YES 유지**. P2 residual(대비 AA·corrupt 교차케이스·axe gate)은 비차단 open.
