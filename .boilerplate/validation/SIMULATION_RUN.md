...
| 58 | **매니페스트 `states[]` 가 상태의 «렌더 조건»을 표현하지 못해, 조건으로 정의된 «못생긴 상태» 의 승인이 vacuous 해진다.** schema v1 의 상태는 `{id, preview, baseline?}` 뿐인데 브리프가 정의하는 상태는 조건을 달고 온다 — 실측 **4건**: `admin-habits/narrow-320`(**뷰포트 320**) · `admin-habits/zoom-200`(**375×812 + 텍스트 2배**) · `today-list/overflow`(**360×800**) · `today-list/long-name`(**textScale 1.3**). 두 화면에 걸쳐 있고 조건 축도 셋이다(뷰포트·textScale·zoom). 기준선 규칙은 «`baseline: true` 상태 × 모든 shot 뷰포트» 라 세 상태 다 **1차·2차 뷰포트에서 기본 textScale 로** 찍힌다. 게다가 320 은 `geometryOnly` 라 스냅샷 대상에서 아예 빠진다. 결과: 「320 에서 3열이 밀리는가」·「360×800 에서 가로 클리핑이 없는가」·「글자 1.3배에서 행 높이가 넘치는가」를 **승인 기준선으로 삼겠다고 적어 놓고 그 조건이 아닌 스냅샷을 승인하게 된다.** 이름과 내용이 어긋난 기준선이다 | **P0** | **R6 실측으로 확정 (2026-09-11) — 예측대로였고 더 나쁘다.** 승인 스냅샷 세트에 `admin-habits-narrow-320-1280x900.png` 와 `-375x812.png` 가 생성됐다. 파일명에 `narrow-320` 과 `1280x900` 이 **함께** 들어 있다. 이미지를 직접 열어 확인한 결과 **1280px 전체 폭의 3열 표가 멀쩡하게 그려져 있다** — 스토리가 선언한 `globals: { viewport: 'admin-320' }` 가 게이트의 `page.setViewportSize` 에 **완전히 무시됐다.** 즉 「320 에서 3열이 밀리는가」의 승인 기준선이 **1280 에서 잘 나온 그림**이다. `zoom-200` 은 `baseline: true` 가 아니라 기준선이 아예 없다. 게이트는 내내 `blockers: 0` 이다. **발견 23 과 같은 형태다 — 승인본이 이름이 말하는 것을 보여 주지 않는다.** 조건이 «틀리게 찍힌» 것이 아니라 «조용히 버려진» 것이라 사람이 스냅샷을 봐도 이상함을 못 느낀다(그림 자체는 정상이다) |
