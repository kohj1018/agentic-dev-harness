...
| 58 | **매니페스트 `states[]` 가 상태의 «렌더 조건»을 표현하지 못해, 조건으로 정의된 «못생긴 상태» 의 승인이 vacuous 해진다.** schema v1 의 상태는 `{id, preview, baseline?}` 뿐인데 브리프가 정의하는 상태는 조건을 달고 온다 — 실측 3건: `admin-habits/narrow-320`(**뷰포트 320**) · `today-list/overflow`(**360×800**) · `today-list/long-name`(**textScale 1.3**). 기준선 규칙은 «`baseline: true` 상태 × 모든 shot 뷰포트» 라 세 상태 다 **1차·2차 뷰포트에서 기본 textScale 로** 찍힌다. 게다가 320 은 `geometryOnly` 라 스냅샷 대상에서 아예 빠진다. 결과: 「320 에서 3열이 밀리는가」·「360×800 에서 가로 클리핑이 없는가」·「글자 1.3배에서 행 높이가 넘치는가」를 **승인 기준선으로 삼겠다고 적어 놓고 그 조건이 아닌 스냅샷을 승인하게 된다.** 이름과 내용이 어긋난 기준선이다 | **P1** | (미정 — 처방 후보: (a) `states[]` 에 선택 필드 `render: { viewports?: [{w,h}], textScale?: number }` 를 두고 있으면 그 조건으로만 찍는다(schema **minor** 추가 — `version` 1 유지, D4 기준선 정의 한 줄 확장) (b) 320 을 `geometryOnly` 에서 빼 스냅샷 대상에 넣는다 — 전 화면 PNG 수가 늘고 textScale 은 여전히 못 푼다 (c) 「조건으로 정의되는 것은 state 가 아니다」를 규칙으로 박고 브리프에서 금지 — 그러면 **그 조건들을 승인 대상에서 빼는 것**이라 게이트의 geometry 검사에만 맡기게 된다. **(a)가 최소 변경이고 브리프가 이미 쓰고 있는 어휘를 그대로 매니페스트로 옮기는 것이다**) |
