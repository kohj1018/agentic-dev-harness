...
- **CLI**: `node design-gate.mjs (--html <files|glob...> | --manifest <path> [--only <id,...>] [--snapshot <dir>] [--no-build] | --self-test [--scopes <dir,...>] | --tokens-only <glob...|dir...>) [--report <path>] [--viewports WxH,...]`. 인자 없음·미정의 플래그 → usage + exit 2. **`--only`에 매니페스트에 없는 id가 오거나 검사 대상 화면이 0개면 exit 2**(조용한 통과 금지). **모듈 해석은 매니페스트 화면 `scope` → cwd → 저장소 루트 순**으로 `createRequire`(monorepo에서 `apps/web/node_modules`만 있는 경우 — `--self-test`·`--html`은 `--scopes` 값을 같은 용도로 쓰고, 자가 검사 (c)가 부르는 매니페스트 모드에는 `--scopes` **전체 목록**을 그대로 넘긴다 — 첫 항목만 넘기면 모듈이 다른 scope에만 설치된 workspace에서 (a)(b) 통과 뒤 (c)만 exit 2가 된다) — `playwright`·`@axe-core/playwright`는 CJS라 `default`까지 본다. **Windows에서는 `npx`·`flutter`가 `.cmd`라 `spawnSync`에 `shell: true`가 필요하다**(Node child_process 문서).
...
- **`--manifest`**: JSON 읽기 → `version !== 1`이면 exit 2. `--only`가 있으면 그 화면만. `profiles[<screen.profile>].viewports` + 항상 `320x720` geometry 전용 추가(웹만). 화면의 `states[]` 각 `preview`(없으면 화면 `preview` 1개)를 렌더한다:
  ...
  - `flutter:<test file>` → 화면의 `scope`(`pubspec.yaml`이 있는 디렉터리 — 기본 `.`)를 작업 디렉터리로 `flutter test <file> --reporter json --dart-define=DESIGN_GATE_OUT=<저장소 루트 design-gate-shots 절대경로>` 실행(**`preview`·`source[]`는 그 scope 기준 상대**, 출력만 루트 절대경로 — cwd가 scope라 상대값을 넘기면 `apps/<scope>/design-gate-shots/`에 쌓여 회수되지 않는다). 테스트 파일 규약(design-milestone R4가 생성): 각 프로필 크기마다 `tester.binding.setSurfaceSize`, `pumpWidget`, `expectLater(tester, meetsGuideline(...))` 4종, `FlutterError.onError` 수집(RenderFlex overflow 등) 0 확인, `captureImage`로 PNG를 `DESIGN_GATE_OUT/<screen>-<state>-<w>x<h>.png` 저장. 러너는 json 이벤트에서 `testDone` 실패를 blocker로 정규화(`rule: guideline:<name> | overflow | exception`)하고, **`testDone` 뒤에 오는 `error` 이벤트·최종 `done.success: false`·프로세스 종료 상태(≠0)도 실패로 집계한다**(testDone만 보면 «성공 뒤 오류»·«exit 1»이 통과로 샌다 — dart test json reporter 프로토콜). `flutter` 미설치·러너 기동 실패·컴파일 오류(스트림에 `protocolVersion` 없음, 또는 `testStart`가 하나도 없는 채 비정상 종료)는 exit 2로 구분한다.
  - **렌더 스크린샷 파일명은 `<name>-<w>x<h>.png`**(폭만 쓰면 같은 폭 다른 높이 뷰포트가 서로 덮어쓴다; Flutter 규약과도 같은 형태가 되어 회수가 하나로 합쳐진다). 렌더 직후 **populated 전제**를 확인해 빈 화면이면 `empty-render` blocker(ADR-058 D3).
  - `--snapshot <dir>`이 있으면 **blocker가 0인 화면만** 기준선 집합(geometry 전용 뷰포트 제외, 각 뷰포트 default + **1차 뷰포트만** empty·error + `states[].baseline: true`)을 PNG 그대로 `<dir>/<screen>-<state>-<w>x<h>.png`로 복사한다(변환 없음). **차단된 화면은 기존 승인본을 덮어쓰지 않는다**(ADR-072 D4). 파일당 500KB 초과는 `report.summary.snapshotWarnings[]`에 경고(차단 아님).
- **`--tokens-only`**: 입력이 디렉터리면 재귀 전개하고 `**` glob을 지원한다(`**`를 먼저 자리표시자로 바꾼 뒤 `*`를 치환한다 — 순서를 지키지 않으면 생성된 정규식이 다시 치환돼 두 단계 이상 깊은 파일이 빠진다). 읽지 못한 입력은 `report.unreadable[]`로 구분해 보고한다(«0건»과 섞이면 검사한 적 없는 것이 통과로 보인다). glob 파일에서 정의 라인(`--<name>: #hex`, `static const Color … =`, tokens 파일 경로) 제외 후 `#hex`·`[#hex]`(Tailwind arbitrary)·`Color(0x…)`·`Colors.<x>`·`\b\d+px\b`(spacing 토큰 정의 밖)를 grep. 결과는 `report.tokens[]`(기록 등급 — exit에 영향 없음).
- **`--self-test`**(4케이스): (a) 내장 known-bad HTML(뷰포트 escape `width: 200vw` 요소 + 대비 2:1 텍스트 + 라벨 없는 `<button><svg/></button>`) 렌더 — **두 fixture 모두 `<title>`·`lang`을 갖춰야 한다**(axe `document-title`은 serious라 known-good이 그 규칙만으로 차단되어 자가 검사가 정상 환경에서도 FAIL한다) → **규칙별** blocker 기대: `page-overflow`(320) ≥1 · `color-contrast` ≥1 · `button-name` ≥1. (b) 내장 known-good HTML → blocker 0. (c) 내장 known-good HTML을 임시 디렉터리에 쓰고 임시 매니페스트(`preview: "url:<path>"`)로 서빙해 매니페스트 경로·report·`--snapshot` 저장까지 exit 0. (d) **Flutter scope마다**(`--scopes`가 있으면 그 목록, 없으면 루트·`*/`·`apps/*`·`packages/*`에서 `pubspec.yaml`+`test/design_gate/`가 함께 있는 디렉터리) `test/design_gate/self_bad_test.dart`(stack-guard가 생성: 20px 탭 타겟 + 2:1 대비 위젯) 실패 + `test/design_gate/self_ok_test.dart` 통과 — 컴파일 오류는 exit 2. 넷 다 기대 일치 → `self-test: PASS` exit 0, 불일치 → `self-test: FAIL` exit 1, 실행 불가 → exit 2.
...
- (e) 6-4-1 절 제목·본문을 교체:
  ```
   - **6-4-1. design gate v3 어댑터 + Visual-QA scaffold (UI 한정 — ADR-072 D6 / ADR-058#amend-3)**:
     ...
     - **자가 검사(4케이스)**: `<pm> validate:design -- --self-test` 실행(Flutter scope가 루트가 아니면 `--scopes apps/mobile`처럼 fixture를 만든 scope를 함께 넘긴다) → 위 판정 행. **capability 버전 핸드셰이크·고정 적합성 oracle은 없다**(ADR-072 D6이 ADR-058#amend-2를 대체). 복사 직후 canonical asset의 SHA-256을 registry `copied-from`에 적는다(caller 대조 없음).
...
- validate-plan 항목 5 `[Plan-FAC-coverage]`의 `**`docs/20-system/prototypes/M<N>/*.html` glob**(`_drafts/` 제외)로 현재 active 화면 HTML 전체를 회수해 — ① … ⑦ …` 문단에서 source를 교체: `M<N>` 입력이면 **검사 대상 화면 = 이 M 매니페스트(`docs/20-system/prototypes/M<N>/manifest.json`, 있으면)의 등록 화면 전부 ∪ 이 M의 feature `프로토타입:` 줄이 가리키는 이전 M 재사용 화면(`M<K>/<screen>` — 그 `M<K>` 파일에서 회수)**이며(ADR-072 D9 — **등록됐는데 어느 feature도 참조하지 않는 화면을 빠뜨리지 않는다**: 그 화면의 orphan PX는 ④가 잡는다), 각 화면의 `source[]` 파일 PX 주석(화면마다 **그 화면이 등록된 M**의 접두로 `grep -rn "PX-M<K>-"`)을 회수해 — ① 각 화면 PX ≥1 · ② 문법·화면 id 일치(`^PX-M<K>-<screen>-\d{2,}$`, `M<K>` = 그 화면이 등록된 M, id의 `<screen>` = 매니페스트 `id`) · ③ 한 화면 source 내 중복 없음 · ④ 코드 PX = 모든 feature `## 7` 인벤토리의 disjoint union(orphan·중복·누락) · ⑤ 각 PX 정확히 1 feature · ⑥ `(id, 설명)` 정확 일치 · ⑦ task `(PX-…)` 태그 ↔ `## 7-3` RHS 일치 — 를 검사한다. **추가 ⑧**: 검사 대상 각 화면의 `snapshots[]` 파일 실재 + `approved.date` 존재 + 각 feature `프로토타입:` id가 **그 id가 가리키는 매니페스트**에 존재(부재 = P0). **전 화면이 이전 M 재사용이면 이 M의 매니페스트 부재는 정상**(P0 금지).` 나머지 문장 유지.
...
- `reviewer.md` Plan Quality 5 `[Plan-FAC-coverage]`의 ``prototypes/M<N>/*.html` glob(`_drafts` 제외)로` → `이 M 매니페스트의 등록 화면 전부 ∪ feature `프로토타입:`이 가리키는 이전 M 재사용 화면(`M<K>/<screen>`은 그 이전 M에서 회수 — ADR-072 D9) + source PX 주석으로`, `HTML` 표현 전부 `코드`로, PX 문법 앵커는 `^PX-M<K>-<screen>-\d{2,}$`(그 화면이 등록된 M 기준), `⑧ 스냅샷·approved·참조 id 실재(참조 id는 그 id가 가리키는 매니페스트에서 확인 — 전 화면 재사용이면 이 M 매니페스트 부재 정상)` 추가. Milestone-Plan Quality 4의 `R5-5` → `design-milestone R7`, `승인 프로토타입` → `매니페스트 화면·브리프`.
...
- `implement-workitem` 3-R (b) `UI면 승인 프로토타입 경로가 **실제로 바뀌었거나 사라졌는지**` → `UI면 — 정확히는 `## 3`에 `승인 UI 재사용` line item이 있으면 — 그 line item의 `manifest: <screen id>`가 가리키는 M(`M<K>/<screen>`이면 그 이전 M)의 매니페스트 화면 entry + `source[]` 파일이 **실재하고 approved 상태인지**(ADR-072 D9 — 재사용 line item이 없는 task(`프로토타입 면제:` feature 등)는 이 점검 대상이 아니다)`. 3-R 뒤 문장 `(참조 프로토타입 경로 삭제·상위 ## 7/INV 변경 등 계획 전제 붕괴)` → `(매니페스트 entry·source 삭제·상위 `## 7`/INV 변경 등)`. `일반 오류(테스트·타입·구현 누락·프로토타입 세부 불일치)` 유지.
- `accept-milestone` R0 2 `## 7의 프로토타입: 참조 줄` → `## 7의 프로토타입: 참조 줄 + **그 참조가 가리키는 M의 매니페스트**(`M<K>/<screen>` 재사용이면 그 이전 M 파일)의 `snapshots[]`·`handoff.run` + 그 화면을 이후 M이 `supersedes`로 재등록했으면 가장 최근 등록·승인본의 값(ADR-072 D5-5 — stabilize §3-V (b)와 같은 기준선 규칙)`; R2 5 `승인 프로토타입 경로(docs/20-system/prototypes/M<N>/<screen>.html)를 함께 제시해` → `R0 2가 회수한 **가장 최근 등록·승인본**의 스냅샷 경로(`docs/20-system/prototypes/M<K>/snapshots/`)와 미리보기 실행 명령(`handoff.run`)을 함께 제시해`. R4/근거의 `프로토타입 경로` → `스냅샷 경로`. R0 2 줄에 `(ADR-072 D9)`·`(ADR-072 D5-5)` 역참조를 붙인다.
...
- `ADR-058` #amend-2 `### 적용 surface`의 `- `.claude/skills/bootstrap-design/SKILL.md`, `.claude/skills/plan-milestone/SKILL.md`: registry command 소비 …` 줄에서 **plan-milestone 경로를 뺀다**(P5-9가 그 skill의 ADR-058 인용을 없애 죽은 역참조가 된다 — 검사 3이 영구히 잡는다): `- `.claude/skills/bootstrap-design/SKILL.md`: registry command 소비 + missing/not-ready hard stop. (당시의 plan-milestone R5 소비자는 ADR-072 D8이 `/design-milestone`으로 옮겼다 — 기록.)`
...
- `GUARDRAILS_STRATEGY.md` stack-guard 산출물 범위 항목 `UI 판정 시에만 JIT canonical asset을 project-native validate:design adapter로 물질화하고 fixed conformance를 실행한 뒤 … capability version·source digest를 기록한다(ADR-058#amend-2)` → `UI 판정 시에만 design gate v3 asset을 project-native `validate:design`으로 물질화하고 자가 검사(4케이스)를 통과시킨 뒤 `## Design Gate Adapter`(6필드)에 기록한다(ADR-072 D6 — ADR-058#amend-2 대체)` — **병기 필수**: 이 파일은 ADR-058 `## Surfaces` 등재 대상이라 `ADR-058` 문자열이 사라지면 부록 E 검사3 backref가 끊긴다. 유지 주기 표에 행 `| UI 화면 승인 시 | `validate:design --manifest` + reviewer 픽셀 판정 + 스냅샷 동결 | `/design-milestone` R6 |`.
...
- 두 README의 **Step 3 빠른 시작 코드블록**(`README.md` 줄 87 / `README_ko.md` 줄 86 — `/plan-milestone` 다음 줄)에 `/design-milestone M1` 한 줄 + UI 전용 주석을 추가한다. **UI 마일스톤은 `/plan-milestone` 뒤 `draft`라 이 줄이 없으면 그대로 따라 한 사용자의 `/plan-workitem M1`이 입구에서 막힌다**(줄 26 흐름도·PROJECT_START_CHECKLIST와 정합).
...
4. `ADR-058#amend-2` 인용도 같은 방식으로 `ADR-072 D6`으로(살아있는 규칙) 또는 병기(기록). **단 ADR-058 `## Surfaces` 등재 파일(`docs/00-meta/GUARDRAILS_STRATEGY.md`·`scripts/README.md` — P6-2에서 이미 처리한 전자 포함)은 `(ADR-072 D6 — ADR-058#amend-2 대체)` 병기형으로 적어 `ADR-058` 문자열을 남긴다** — 단순 치환하면 검사3 backref가 끊긴다(대안: ADR-058 `## Surfaces`에서 그 줄을 제거 — 소유 이관 반영이지만 fan-out SSOT 본문 수정이라 병기를 기본으로 한다).
   ```bash
