...
## 배경
- [관측됨] `.claude/agents/` 8종(architect / builder / validator / planner / reviewer / qa / researcher / designer)은 전부 *개발 lifecycle 역할*이다. 법률·사업전략·마케팅·데이터분석 판단의 자리가 없다. 보안은 코드 취약점 경로(`/security-review`·secret scanner 권장 ADR-021#amend-1·추적 시크릿 점검 ADR-059 D9)만 있고 **설계층 위협 모델**이 빈다.
...
동시 호출은 금지된다(arXiv 2602.01011 — 조율 오버헤드). 2개 이상 필요하면 순차이며 순서는 **입력을 만드는 쪽이 먼저**다. 두 번째 호출을 사용자가 결정하므로 A→B→A 가 무한 루프가 아니라 사용자 판단 지점이 된다.
...
- **작성하는 노트에도 같은 규율을 적용한다** — 노트가 전문가끼리 쓰는 문서가 되면 6개월 뒤 작성자 자신이 읽지 못한다.
...
## 정책 강도 (ADR-022)
- **제약(강) — [외부실증]**: D2(지식은 조회, 페르소나는 규율)·D4(문서 경유)·D5(재자문 반환). 세 논문 실증에 근거하며 agent 행동을 좁힌다.
...
- arXiv:2602.01011 (멀티에이전트 팀의 조율 오버헤드 — D5 동시 호출 금지 인용 owning)
...
4. **해당 agent 1개에 `Agent` 위임.** **여러 도메인을 동시에 부르지 않는다**(ADR-062 D5 — 조율 오버헤드). 2개 이상 필요하면 순차이며 **입력을 만드는 쪽을 먼저** 부른다.
...
- **작성한 노트에도 같은 규율을 적용한다** — 노트가 전문가끼리 쓰는 문서가 되면 6개월 뒤 작성자 자신이 읽지 못한다.
...
agent 가 `재자문 필요: <도메인> — <무엇을 왜>` 를 반환하면 **자동으로 그 도메인을 부르지 않는다.** 사용자에게 제시하고 결정을 받는다.
...
| 1차 | 국가법령정보 공동활용 Open API `open.law.go.kr` | **사용자가 발급·제공한 키가 있을 때만.** agent 가 키를 발급·저장하지 않는다(ADR-062 D11) | 법령·자치법규·판례 구조화 조회 | `[확인됨-조문]` |
...
**MCP·Open API 가 모두 없는 기본 환경의 실제 능력 (정직 고지)**:
...
- **출력 첫 부분에 조회 환경을 명시한다** — `조회 환경: 법령 MCP 미연결 — [확인됨-조문] 불가, 가이드라인 경로만 사용`. 이 줄이 없으면 등급의 의미가 왜곡된다.
...
## 출력 계약 (ADR-062 D7)
첫 줄은 결론 1문장(두괄식). 형식은 ADR-060 D3 Decision Brief 6블록 — **용어 블록 필수**(법률 용어를 처음 보는 사람이 배경만 읽고 이해되게). 압축 예외 적용(ADR-046#amend-1) — 짧게 줄이지 않는다. 노트에도 같은 규율을 적용한다.
...
3. **CAC/LTV 를 실측 없이 예측하지 않는다. 역산으로 전환한다.**
   - ❌ `CAC 5만원, LTV 30만원 예상`
   - ✅ `가격 9,900원 + 월 이탈률 5% 가정이면 LTV = 19.8만원. → CAC 가 6.6만원(LTV 1/3)을 넘으면 이 모델은 성립하지 않는다. → 물어야 할 것: "6.6만원 안에 고객 1명을 얻을 채널이 있나?"` (채널 CAC 벤치마크는 `marketing` 도메인으로 라우팅)
...
## 출력 계약 (ADR-062 D7)
첫 줄은 결론 1문장(두괄식). ADR-060 D3 Decision Brief 6블록 — **용어 블록 필수**(사업 용어를 처음 보는 사람이 이해되게: CAC=고객 1명을 얻는 데 드는 비용, LTV=고객 1명이 평생 주는 매출 등). 압축 예외 적용. 노트에도 같은 규율.
...
## 출력 계약 (ADR-062 D7)
첫 줄은 결론 1문장(두괄식). ADR-060 D3 Decision Brief 6블록 — **용어 블록 필수**(포지셔닝·JTBD·objection 같은 용어를 처음 보는 사람이 이해되게). 압축 예외 적용. 노트에도 같은 규율.
...
## 출력 계약 (ADR-062 D7)
첫 줄은 결론 1문장(두괄식). ADR-060 D3 Decision Brief 6블록 — **용어 블록 필수**(코호트·퍼널·유의성·표본 같은 용어를 처음 보는 사람이 이해되게). 압축 예외 적용. 노트에도 같은 규율.
...
## 출력 계약 (ADR-062 D7)
첫 줄은 결론 1문장(두괄식). ADR-060 D3 Decision Brief 6블록 — **용어 블록 필수**(위협 모델·인증/인가 차이·최소 권한 같은 용어를 처음 보는 사람이 이해되게). 압축 예외 적용. 노트에도 같은 규율.
...
| 설계층 보안 (위협 모델·보호 등급·인증/인가 경계) | security | `/consult-expert security <대상 자산>`. **코드 취약점 스캔·secret 검출은 범위 밖**(기존 경로). 위협 표 전 칸(**잔여 위험 포함**) 필수. 구조 결정은 architect 소유. report-only (ADR-062) |
```

**그리고 위임 표 아래 노트 단락에 1줄 추가** (47행 `> **검증 위임 규율 (ADR-050#amend-1)**:` 문단 뒤):
```
> **도메인 자문 규율 (ADR-062)**: 위 도메인 agent 5종은 *계획 이전* 단계의 자문이다 — 구현 결과를 감사하지 않는다(감사는 validator/reviewer/qa 소유). **동시에 2개 이상 부르지 않는다**(조율 오버헤드 — arXiv:2602.01011). 도메인 A 의 결론은 정본 문서·원장에 기록된 뒤에야 B 가 읽는다(agent 간 직접 통신 금지). 남의 소유 사실을 고쳐야 하면 `재자문 필요: <도메인>` 을 반환하고 사용자가 결정한다.
```
