...
- validate-plan 항목 5 `[Plan-FAC-coverage]`의 `**`docs/20-system/prototypes/M<N>/*.html` glob**(`_drafts/` 제외)로 현재 active 화면 HTML 전체를 회수해 — ① … ⑦ …` 문단에서 source를 교체: `M<N>` 입력이면 **이 M의 feature `프로토타입:` 줄이 가리키는 매니페스트 전부**(`<screen>`은 `docs/20-system/prototypes/M<N>/manifest.json`, `M<K>/<screen>` 재사용은 그 `M<K>` 파일 — ADR-072 D9)에서 참조된 화면과 각 `source[]` 파일의 PX 주석(화면마다 **그 화면이 등록된 M**의 접두로 `grep -rn "PX-M<K>-"`)을 회수해 — ① 각 화면 PX ≥1 · ② 문법·화면 id 일치(`^PX-M<K>-<screen>-\d{2,}$`, `M<K>` = 그 화면이 등록된 M, id의 `<screen>` = 매니페스트 `id`) · ③ 한 화면 source 내 중복 없음 · ④ 코드 PX = 모든 feature `## 7` 인벤토리의 disjoint union(orphan·중복·누락) · ⑤ 각 PX 정확히 1 feature · ⑥ `(id, 설명)` 정확 일치 · ⑦ task `(PX-…)` 태그 ↔ `## 7-3` RHS 일치 — 를 검사한다. **추가 ⑧**: 참조된 각 화면의 `snapshots[]` 파일 실재 + `approved.date` 존재 + 각 feature `프로토타입:` id가 **그 id가 가리키는 매니페스트**에 존재(부재 = P0). **전 화면이 이전 M 재사용이면 이 M의 매니페스트 부재는 정상**(P0 금지).` 나머지 문장 유지.
...
- `reviewer.md` Plan Quality 5 `[Plan-FAC-coverage]`의 ``prototypes/M<N>/*.html` glob(`_drafts` 제외)로` → `feature `프로토타입:`이 가리키는 매니페스트 전부(재사용 `M<K>/<screen>`은 그 이전 M — ADR-072 D9) + source PX 주석으로`, `HTML` 표현 전부 `코드`로, PX 문법 앵커는 `^PX-M<K>-<screen>-\d{2,}$`(그 화면이 등록된 M 기준), `⑧ 스냅샷·approved·참조 id 실재(참조 id는 그 id가 가리키는 매니페스트에서 확인 — 전 화면 재사용이면 이 M 매니페스트 부재 정상)` 추가. Milestone-Plan Quality 4의 `R5-5` → `design-milestone R7`, `승인 프로토타입` → `매니페스트 화면·브리프`.
