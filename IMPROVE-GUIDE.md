...
- **판정 계약 — 이 순서대로 본다.** 순서가 중요하다: 앞 단계에서 걸리면 뒤 단계를 보지 않는다. **순서를 뒤집으면 환경 문제를 "테스트 없음"으로 오분류한다**(아래 실측 참조).
  1. **스트림이 성립하는지** — 첫 `start` 이벤트에 `protocolVersion`이 있는가. 없거나(러너가 기동조차 못 함) 파서가 모르는 major 버전이면 **`BLOCKED_ENV`** 로 확정하고 여기서 멈춘다. `stderr`에 원인(예: `No devices are connected`)이 있으면 함께 보고한다. **이때 `EMPTY`로 분류하지 않는다** — 테스트가 없는 것이 아니라 실행 자체가 불가능한 것이고, 처방이 다르다(작성 vs 환경 복구).
  2. **e2e가 실제로 돌았는지** — `suite` 이벤트의 `path`를 정규화(구분자 통일)했을 때 **선언된 e2e 디렉터리 하위**인 suite가 1개 이상인가. 0개면 **`EMPTY`**. **이 항이 실측 결함의 차단 지점이다** — 빈 디렉터리를 지정하면 러너가 유닛 테스트 디렉터리를 대신 실행하고 `done.success=true` + exit 0을 내는데, 그때 suite 경로가 e2e 디렉터리 밖이라 여기서 걸린다.
  3. **그 suite의 테스트가 성공했는지** — 그 suite에 속한(`testStart.test.suiteID`가 그 suite `id`) 테스트 중 `testDone.testID`가 같은 것이 `hidden=false` · `skipped=false` · `result="success"`인 것이 1개 이상인가. 0개면 **`FAIL(project)`**.
...
> ```
>
> 작성 시점 실측: **007=5개(다음 6)** / 014=3(다음 4) / **021=1개(다음 2)** / 027=7(다음 8) / 031=0(다음 1) / 052=0(다음 1). 아래 지시는 이 값을 전제로 쓰였다 — 다르면 실제 값으로 바꾼다.
...
- 파일 맨 끝에 amendment를 추가한다. **번호는 6**이다(이 ADR엔 이미 1~5가 있다).
