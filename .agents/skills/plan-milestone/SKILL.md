...
**Sub-agent parity**: 본문 R2의 architect 단발 sub-call은 Claude `Agent` 도구 기능이다. Codex 서브에이전트는 GA이나 본 저장소가 Claude persona 위임을 Codex subagent로 아직 매핑하지 않아 메인 세션이 순차 단일 실행으로 직접 추론한다(degrade — SKILL.md 본문의 Codex 노트 정합).
