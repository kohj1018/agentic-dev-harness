...
**Slash command translation**: 본문 grep 실측상 마지막 출력·비교 문맥에 등장하는 슬래시 커맨드 4개를 각각 치환해 안내한다 — `/validate-discovery`→`$validate-discovery`, `/bootstrap-project`→`$bootstrap-project`, `/plan-workitem`→`$plan-workitem`, `/repair-plan`→`$repair-plan`. Codex CLI는 `/`를 빌트인 슬래시 커맨드에 쓰므로 명시적 치환이 필요.
